...
**142 scripts** across 9 categories.
...
| `check-branch-ancestry.mjs` | on `origin/main`. | — | pre-commit · warn |
| `check-circular-deps.mjs` | Builds the intra-`src/` static import graph (relative + `@/*` alias imports), finds every import cycle via Tarjan's strongly-connected-components algorithm, | — | manual · error |
...
| `check-guardrail-drift.mjs` | Shared guardrail harness scripts (proof-of-firing, run-gates, the registry validator, …) are vendored into multiple repos (ops / hds / site-engine). When one repo fixes a bug in a shared script, the fix can silently… | — | ci-pr · error |
| `check-handoff-freshness.mjs` | (no header description) | `check:handoff-freshness` | manual · warn |
...
| `check-parked-triggers.mjs` | check-parked-triggers — evaluate the re-entry triggers in docs/ai/PARKED.md. | `parked:check` | pnpm-meta · warn |
...
| `check-skills-lock.mjs` | Drift gate for skills-lock.json: for every pinned skill, asserts it is installed under `.claude/skills/<id>/` and its files hash-match the lock. Read-only, no network — run `node scripts/install-skills.mjs` first to | `check:skills-lock` | manual · error |
...
| `check-steering-budget.mjs` | Every file in docs/guardrails/steering-budget.json is loaded by every agent in every session before it reads a line of code. On 2026-09-14 that set measured | — | pre-commit · error |
...
| `audit-orphan-modules.mjs` | (no header description) | `audit:orphan-modules` | pnpm-meta · warn |
...
| `audit-strengths.mjs` | Verifies that each documented differentiator in docs/architecture/strengths-and-differentiators.md is still real. | — | ci-scheduled · warn |
...
| `generate-lead-site.mjs` | The metered path (`api/lead-action.ts` action 'generate' → lib/agent) bills ANTHROPIC_API_KEY tokens per lead. This path doesn't call the API at all: a | — | — |
| `generate-portal-token.mjs` | generate-portal-token — mint a /c/:slug?token=… HMAC token for a client. | — | — |
| `generate-pwa-icons.mjs` | references, with zero image-library dependencies (raw PNG encoding via zlib). | — | — |
...
| `digest-middleware.mjs` | /api/digest-action. Reuses lib/digests/actions.mjs + lib/supabase/digests.mjs (same logic as the prod functions). Wired in vite.config.mjs with apply:'serve' so it never ships to prod. | — | — |
...
| `export-call-list.mjs` | Read-only. Never writes to Supabase; logging an outcome is scripts/log-call.mjs. | — | — |
...
| `log-call.mjs` | Without this, 200 calls produce anecdotes. With it they produce a funnel you can divide: dials -> reached -> interested -> booked. | — | — |
| `log-run.mjs` | (no header description) | — | — |
| `meeting-to-tasks.mjs` | meeting transcript, append them to docs/ai/proposed-units.jsonl. | — | — |
| `metric-north-star-share.mjs` | North-star share metric (ops#293, decided in the ops#274 frontier-engineering session). Over a rolling window (default 14 days), the share of merged PRs | `metric:north-star-share` | manual · warn |
...
| `promote-learned-rule.mjs` | Interactive walker over docs/ai/learned-rules.jsonl. For each unpromoted entry (no `promotedAt` field), prompts Adrian to promote it to a real registry entry in docs/guardrails/registry.json. The actual flip from | `guardrail:learned-rules`, `guardrail:promote-rule` | — |
...
| `seed-digest-items.mjs` | digest_items store (Digest P1, ops#78). | — | — |
