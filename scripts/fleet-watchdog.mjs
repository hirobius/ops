 ...
 *     write seam (ops#139) — `redispatch`/`flag` actions do the actual
 *     `dispatch_count`/`last_dispatched_at`/`dispatch_status`/`status` writes;
 *     this script no longer touches those columns directly.
...
/**
 * Re-dispatch one stale task via the `redispatch` action (lib/tasks/actions.mjs,
 * ops#139) — the field writes (dispatch_count/last_dispatched_at/dispatch_status)
 * and the optional re-ping comment both happen behind that one seam; this
 * function only notifies + logs the run recap around it.
 */
export async function reDispatch(sb, github, { task, comment, maxRetries }) {
  ...
  if (result.body.commentError) {
    const hint = /GITHUB_TOKEN not set/.test(result.body.commentError)
      ? ` Set it at ${VERCEL_ENV_URL}.`
      : '';
    console.log(`${task.key}: ${result.body.commentError}${hint}`);
  }
 ...
 * (lib/tasks/actions.mjs, ops#139) — block it, notify + log.
 */
