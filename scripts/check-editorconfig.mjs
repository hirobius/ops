 ...
 *
 * Fixture-proof-of-firing mode (validate-fixture-proof-of-firing.mjs): with
 * --fixture-mode and FIXTURE_FILE set, runs `node <FIXTURE_FILE>` in place of
 * the real editorconfig-checker binary, so fixtures/check-editorconfig/*.mjs
 * exercise this same code path deterministically (no network involved).
 *
...
function main() {
  const fixtureMode = process.argv.includes('--fixture-mode') && process.env.FIXTURE_FILE;

  const bin = fixtureMode ? 'node' : process.env.CHECK_EDITORCONFIG_BIN || 'pnpm';
  const args = fixtureMode
    ? [process.env.FIXTURE_FILE]
    : (process.env.CHECK_EDITORCONFIG_ARGS || 'exec editorconfig-checker')
        .split(' ')
        .filter(Boolean);
