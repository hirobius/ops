...
describe('assemble', () => {
  ...
  it('never emits remote photo URLs into gallery, only local public/ paths', () => {
    const lead = { ...BASE_LEAD, photos: ['https://example.com/a.jpg', '/photos/local.jpg'] };
    const config = defineClient(assemble(lead, CONTENT));

    expect(config.gallery).toEqual([{ src: '/photos/local.jpg', alt: 'Acme Roofing photo' }]);
  });

  it('leaves the gallery empty when no photos are present', () => {
    const config = defineClient(assemble(BASE_LEAD, CONTENT));
