...
| ⑤   | Preview / portal           | 🟢 live · in portal-kit    | Client portals moved to `hirobius/portal-kit` (own password-gated deployment per client, e.g. lilac → lilac-insure.vercel.app). The in-ops `/c/:slug?token=` portal + its HMAC gate (`lib/portal-auth.mjs`, `api/portal-verify.ts`, #28) were **removed** (the route was never visited). `preview_url` remains the outreach hook. Env `PORTAL_HMAC_SECRET`/`VITE_PORTAL_HMAC_SECRET` are now unused (safe to remove in Vercel). |
