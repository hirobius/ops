...
| ⑧   | Deal → invoice             | 🔴 not built · decision made | **Decision 2026-07-13: Stripe no-code, don't build** — Payment Link (build fee) + Subscription (~$79/mo care plan); publish stays the billing event, Stripe is the mechanism. Runbook + gap analysis: `docs/CLIENT-READINESS.md`. Optional later: record `invoice_url`/status on the client row.                                                                     |
