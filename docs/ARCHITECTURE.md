...
(+ Runs panel). The pre-Ralph `/admin/approvals` inbox (epic #41 Slice 3) was
retired (ops#157) — merge governance now runs entirely through ralph-gate's
`ralph-auto`/`ralph-approved` labels via the board's "Approve merge"/"Auto-merge"
menu items; the dead `localhost:3005/orchestration/*` bridge is fully retired
too. Auth:
