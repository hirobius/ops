...
| ③ | Generate site | 🔵 wired, never run | `lib/agent/` (enrich→generate→judge, `llm.mjs` MODELS), `ClientConfig` in `lib/schema`; trigger `/api/lead-action` action `generate` → status `scored`, writes config. Env: `ANTHROPIC_API_KEY` (set, unexercised). Output is a *config*, not a live site. |
...
(+ Runs panel) · admin/approvals (still on the dead orchestration bridge). Auth:
...
Claude). Run-log: `docs/ops/run-log.jsonl` + Runs panel (#8 half). Gaps: approvals
inbox, autonomy dial, tier→model (#13).
