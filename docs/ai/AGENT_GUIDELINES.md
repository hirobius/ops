...
- `docs/security/sbom.json` — generated by the SBOM gate. Rebaked on any
  `pnpm install` / gate run, and the diff carries a fresh `serialNumber`,
  a new `timestamp`, and whatever npm version the runner happens to have —
  100k+ lines of churn that says nothing about the dependency tree. Standing
  order: `git checkout -- docs/security/sbom.json` on sight; never commit a
  regen as part of unrelated work. (Added 2026-09-14 after a gate run in a
  sandbox with older npm produced a 111,574-line false diff.)
