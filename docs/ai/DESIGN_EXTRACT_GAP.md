...
## Addendum (2026-07-12): SkillUI — adjacent tool

[SkillUI](https://github.com/amaancoderx/npxskillui) (`npx skillui --url <site>`) covers the same source-free URL-crawler gap as design-extract — it reverse-engineers a live site into a Claude-ready design-system skill (tokens, fonts, screenshots, SKILL.md) via pure static analysis. Cataloged on `/ops/skills` (deliberately not vendored — generator CLI, run ad hoc). Before either becomes the client-onboarding default, run the tracked skillui-vs-extract-design bake-off on one prospect site and fold the verdict back into this doc.
