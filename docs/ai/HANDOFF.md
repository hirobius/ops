...
'queued'` — proposed for dispatch but awaiting a human click, distinct
  ...
  queued-count Approvals tile. **Was not built (out of Slice 3 scope), now
  closed by Slice 6 below:** the mayor/agent side of "queued" — see the new
  Slice 6 bullet. See Done-log for full verification.
- **NEW: Fleet auto-dispatch epic #41, Slice 6 landed (2026-07-08) — closes
  issue #8.** `scripts/fleet-dispatch.mjs` now auto-queues: every run also
  proposes up to `--queue-max` (default 5) open, non-`auto_ok`,
  not-yet-dispatched/queued tasks into `/admin/approvals`
  (`dispatch_status='queued'`, via the existing `queue` action) — completing
  the autonomy dial ("`auto_ok` tasks self-dispatch and recap; everything
  else queues for a click", no operator toggle needed first). A task Adrian
  denies (`unqueue` resets `dispatch_status` to null) is never
  auto-re-queued: `priorApprovalTaskKeys()` reads `docs/ops/events.jsonl` for
  a prior `approval_waiting` event on that key. New `ApprovalsPanel.tsx`
  renders the live queued-tasks inbox with inline Approve/Deny directly on
  `/ops` (previously only a SurfacesRail link-tile) — `AgenticOSPage.tsx`
  now owns one `useApprovalsInbox()` poll shared between the tile's count and
  the panel (no duplicate polling). `lib/ops/run-log.mjs` +
  `scripts/log-run.mjs` gain an optional numeric `tokens` field (`--tokens`
  flag), rendered on the Runs panel — closes the "task, model tier, tokens,
  outcome" recap ask. `docs/ARCHITECTURE.md` + `docs/pipeline-walkthrough.html`
  updated in lockstep. See Done-log for full verification.
...
2. **Task importer — Slice 1 landed (2026-07-07), Slice 2+ open.** Slice 1
   ...
   - pure `lib/tasks/tier.mjs::routeTask` (mechanical|standard|judgment →
     ...
     **Slice 6 landed (2026-07-08) — closes issue #8:** `fleet-dispatch.mjs`
     auto-queues non-`auto_ok` tasks into `/admin/approvals`, and the
     approvals inbox + a `tokens` recap field render directly on `/ops`; see
     the new Now-section bullet + Done-log. **Still open:** neither
     `fleet-dispatch.mjs` nor `fleet-watchdog.mjs` is wired into a live
     cron/Routine yet — both remain manual-trigger, documented as intended
     to run inside the mayor Routine (Adrian's call on when to automate the
     trigger itself, per the Y6 manual-trigger-only standing rule).
...
- 2026-07-08 (Fleet auto-dispatch epic #41, Slice 6 — auto-queue, closes issue #8): closed the gap Slice 3 explicitly left open ("nothing sets `dispatch_status='queued'` automatically"). `scripts/fleet-dispatch.mjs` gains a second, independent pass alongside the existing auto-dispatch one: pure `selectToQueue(tasks, seenTaskKeys, {max})` filters to `auto_ok!==true && !dispatch_url && status==='open' && !dispatch_status && key not in seenTaskKeys`, capped at `--queue-max` (default 5); on `--apply` each selected task is queued via the existing `applyTaskAction(..., {action:'queue'})` (no new action, reuses Slice 3's SIMPLE handler), then `notifyEvent({kind:'approval_waiting'})` + `appendRun({outcome:'queued'})`. The queue pass runs even when `GITHUB_TOKEN` is unset (unlike dispatch, it never calls GitHub) so a missing token never silently blocks the whole run. **Anti-spam correctness fix caught before shipping:** `unqueue` (deny) resets `dispatch_status` to `null` — identical to "never proposed" — so a naive re-run would re-queue a task Adrian just denied. Fixed via `priorApprovalTaskKeys()`, which reads the already-committed `docs/ops/events.jsonl` for a prior `approval_waiting` event on that task key and excludes it; a denied task only returns to the inbox if a human re-queues it by hand or flips `auto_ok`. **Frontend:** new `src/app/pages/ops/agentic-os/ApprovalsPanel.tsx` renders the live `dispatch_status==='queued'` inbox (title, tier/model badge, Approve/Deny) directly on `/ops` as a `Section`, between PillarRail and Fleet — previously the only trace on `/ops` itself was a SurfacesRail link-tile with a count. `AgenticOSPage.tsx` now calls `useApprovalsInbox()` once and threads the result into both `SurfacesRail`'s count prop and `ApprovalsPanel`'s `inbox` prop, so adding the panel does NOT double the `/api/tasks` poll rate. **Recap fields:** `lib/ops/run-log.mjs`/`scripts/log-run.mjs` gain an optional numeric `tokens` field (`--tokens N`, dropped silently if non-finite/negative rather than throwing) — `RunsPanel.tsx` renders it (`45,200 tok`) alongside task/model — closing the issue's explicit "task, model tier, tokens, outcome" recap ask; no caller populates it yet (`fleet-dispatch.mjs` only opens/flips GitHub issues, it doesn't run a model, so it has no token count to report — a future session/subagent wrapper that DOES have a token count is the intended caller). `docs/ARCHITECTURE.md` + `docs/pipeline-walkthrough.html` updated in lockstep (autonomy-dial gap closed, Slice 6 chip added). **New tests:** `scripts/__tests__/fleet-dispatch.test.mjs` gains a `selectToQueue` describe block (empty/non-array input, each filter condition independently incl. the seen-keys exclusion and a plain-array-as-Set convenience path, default/custom/zero/non-finite `--queue-max`, and a check that an `auto_ok` task is never double-selected by both passes); `scripts/__tests__/run-log.test.mjs` gains tokens-valid and tokens-dropped-when-invalid cases. **Verification limitation (real, disclosed, not glossed over):** this session's sandbox permission mode declined every `node`/`pnpm` invocation (`pnpm exec vitest run …`, `pnpm typecheck`, even bare `pnpm --version`) as "requires approval" with no interactive user available to grant it — so typecheck/lint/build/vitest could NOT be run this session, unlike every prior Slice's report. All changes were reviewed by hand (line-by-line re-read after each edit, cross-checked against `lib/tasks/actions.mjs`'s `queue`/`unqueue` contracts, `lib/ops/notify.mjs`'s `EVENT_KINDS`, and the existing `selectAndRoute`/`isMissingColumnError` test style) but genuinely unverified by tooling — **Adrian should run `pnpm typecheck && pnpm exec vitest run && pnpm exec vite build` on this branch before merging**, and treat this PR with more scrutiny than usual until that's green.
