...
0. **⚠️ Fix the ralph-gate outage (ops#144) — everything Ralph is behind it.**
   `hirobius/ralph@main` @ `7387f7c8` startup-fails every gate org-wide; revert
   to `03dea1df` or fix the YAML (needs a human or a session scoped to
   hirobius/ralph), then remove + re-add `ralph-approved` on ops PR #133 (DS
   0.13 bump) to merge it. Then: (a) review/merge the **#136 declutter PR**
   from `claude/ops-tasks-ralph-cockpit-bcfo31`; (b) apply migration
   `0010_task_source_url.sql` in Supabase and click "Import GitHub issues" on
   /ops/tasks once #99 lands; (c) still open from before: branch protection
   for `hds` + `site-engine` (required check `ralph-gate` + Allow auto-merge).
   PR #124 merged pre-outage — that item is done.
...
- 2026-07-11 (Adrian, cockpit session): **everything ralph-auto** — batch
  ...
  back to human-approved by removing `ralph-auto`. (hds/site-engine issues
  need the same label pass — outside this session's repo scope.)
