...
- **⚠️ Ralph loop wedge (2026-07-10) — diagnosed, cleared, hardened; ONE DEFECT LEFT (next thread).** The loop wasn't "not firing" — the single-flight guard in `.github/workflows/ralph.yml` skips whenever any `ralph/*` PR is open, and two were stuck: **#114** (ralph-gate=failure; the AI reviewer was RIGHT — the #99 reconcile mass-soft-deletes the board if `listOpenIssues()` truncates at 100/returns empty; requirement re-scoped onto #99, PR closed) and **#95** (stale base, no gate; rebased onto main + DOM-node baseline re-locked → gate PASS + AI-approved → **MERGED 23:15Z**). The silent-skip problem is fixed on **`claude/ralph-firing-issue-xqwpf9`** (guard classifies wedged vs healthy in-flight + loud Discord alert) — **merge that branch to activate**. **REMAINING DEFECT (confirmed empirically):** the push-to-main chain NEVER self-fires after an auto-merge — `ralph-gate.yml` merges via `gh pr merge` with the default `GITHUB_TOKEN`, and GitHub suppresses `push` workflow triggers for GITHUB_TOKEN-created events (recursion prevention). Zero Ralph runs fired in the 25 min after #95 merged; every past "chain" run was actually a human push. **Fix for the next thread:** have ralph-gate merge with a PAT or GitHub-App token (e.g. `RALPH_CHAIN_TOKEN` secret) so the merge push fires `ralph.yml`, OR have ralph-gate explicitly `workflow_dispatch` ralph.yml after a successful merge (cheaper, no new secret scope). A manual `workflow_dispatch` was fired 2026-07-10 ~23:40Z to start draining the ~18-issue `ralph-ready` queue one iteration at a time.
...
- 2026-07-10 (Ralph wedge CLEARED + chain-token defect confirmed): #114 closed (reviewer's mass-soft-delete finding was real; pagination/empty-guard re-scoped as hard requirement on #99 — see issue comment); #95 rebased onto main (union-resolved registry.json + progress.txt conflicts, both gates additive), DOM-node baseline re-locked (TasksPage 53→54, drift from the tasks-card rework on main — not a #95 regression), full gate green locally (typecheck+456 tests+lint) → ralph-gate PASS + AI-approved → auto-MERGED 23:15Z, closing the #69 guardrails epic (all 3 slices shipped). Then confirmed the chain defect: zero push-triggered Ralph runs after the bot merge — GITHUB_TOKEN-created pushes don't fire `push` workflows, so the auto-merge→next-iteration chain has never worked; fix = PAT/App token for the merge, or ralph-gate dispatches ralph.yml post-merge (see Now bullet). Manual workflow_dispatch fired to start the queue. Branch `claude/ralph-firing-issue-xqwpf9`.
- 2026-07-10 (Ralph-firing diagnosis + wedge-detection guard): traced "Ralph isn't firing" to the single-flight guard in `.github/workflows/ralph.yml` no-op'ing every run because two `ralph/*` PRs are stuck open (#114 ralph-gate=failure/changes-requested + no self-heal loop; #95 stale human-opened ralph-branch PR, no gate status) — with 19 `ralph-ready` issues queued behind them. The plain skip fired **no** notification (Discord step gated on non-skip), so the wedge was invisible. Hardened the guard: it now fetches each open Ralph PR's latest `ralph-gate` commit status, classifies `failure` or `none/pending`-and-≥3h-old as **wedged** (vs a fresh healthy in-flight PR = quiet skip), sets `wedged`/`reason` outputs, and a new "Alert Discord" step pings loudly naming the blocking PRs + the fix ("address the review, rebase, or close"). Single-flight preserved — never stacks a second PR. Classification smoke-tested locally against #114/#95/fresh-PR fixtures (correct: 2 wedged, 1 quiet). YAML validated. Immediate unblock (clear #114/#95) left to Adrian as an operational call — the fix makes future wedges self-announce. Branch `claude/ralph-firing-issue-xqwpf9`.
