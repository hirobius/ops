...
0. **Ralph hardening one-time setup (Adrian, ~5 min).** (a) Merge the
   `claude/ralph-loop-hardening-p1k3a7` PR (close the superseded
   `claude/ralph-firing-issue-xqwpf9` one). (b) Share the org secrets
   `CLAUDE_CODE_OAUTH_TOKEN` + `DISCORD_WEBHOOK_URL` with private repos:
   <https://github.com/organizations/hirobius/settings/secrets/actions>.
   (c) Allow org repos to call ops' reusable workflows:
   <https://github.com/hirobius/ops/settings/actions> → Access →
   "Accessible from repositories in the `hirobius` organization".
   (d) Per consumer repo when rolling out: branch protection with required
   check `ralph-gate` + "Allow auto-merge" (`ralph/README.md` step 4).
   From then on: tag issues `ralph-ready` (+ `p0`–`p3`, + `ralph-auto` to
   batch-pre-approve merges); approve other PRs by labeling `ralph-approved`.
