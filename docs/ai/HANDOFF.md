...
6. **clients repo cleanup** — tracked canonically as **hirobius/clients#10**
   ...
   can't diverge.
7. **Tasks/issues consolidation (#52) — direction locked 2026-07-08, build
   deferred.** GitHub issues are the canonical unit for anything an agent
   touches; `tasks` is the index/control-plane. Full 4-slice plan (schema
   split for `source_url`/`dispatch_url`, confirm cross-repo token scope,
   agent-ready task shape + dispatch-body template, retire `/ops/issues`
   last) at `docs/operations/tasks-issues-consolidation-plan.html`. Open
   question for Adrian before a session builds Slice 1: split
   `source_url`/`dispatch_url` into two columns (recommended) vs. collapse
   to one identity + a `dispatch_status` value — see the plan doc §4.
...
- 2026-07-08 (#52 tasks/issues consolidation — decision + plan, no build): the issue itself frames this as "deferred build; direction locked here," so rather than rush a multi-slice schema/UI change, verified every factual claim in the issue against the actual code (Explore-agent survey, not guesswork) and turned it into an executable plan. Confirmed: the importer (`lib/github/issues.mjs#listOpenIssues()`) already pulls from every repo the `GITHUB_TOKEN` identity can see (GitHub's `/issues?filter=all` feed, not scoped to `hirobius/ops`) — so the DoD's "cross-repo visibility" item is mostly already true, just needs token-scope confirmation, not new scanning code. Confirmed the `source_url`/`dispatch_url` conflation is real and has a concrete failure mode: `import-issues.mjs` writes an imported issue's own URL into `dispatch_url`, and `fleet-dispatch.mjs`/`fleet-watchdog.mjs` both treat `dispatch_url IS NULL` as "not yet dispatched" — so an imported task can never become eligible for auto-dispatch. Wrote `docs/operations/tasks-issues-consolidation-plan.html` (self-contained plan doc, per CLAUDE.md §1's convention for substantial plans): current-state findings, two named options for the schema fix (recommend splitting into `source_url`+`dispatch_url` — additive, lower-risk than collapsing to a status enum), the agent-ready task shape gap (no `description`/`acceptance_criteria` columns exist today — `dispatch()` only carries title/lane/deps/notes into the `@claude` issue body), and a 4-slice build order ending with retiring `/ops/issues` last. Recorded the locked decision in `docs/ARCHITECTURE.md` + `docs/pipeline-walkthrough.html` in the same commit (lockstep rule). **Not built (deliberate — matches the issue's own "deferred"):** no migration, no code changes to the importer/dispatcher/board, `/ops/issues` still live. Two open questions left for Adrian in the plan doc §8 before a session picks up Slice 1.
