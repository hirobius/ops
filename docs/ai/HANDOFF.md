...
- **⚠️ Ralph loop wedged (2026-07-10) — diagnosed + hardened.** The loop wasn't "not firing" — every label/push event fires it, but the single-flight guard in `.github/workflows/ralph.yml` skips whenever any `ralph/*` PR is open, and **two are stuck open**: **#114** (`ralph/issue-99`, bot) has `ralph-gate=failure` — the AI reviewer flagged a real defect (the #99 reconcile mass-soft-deletes the board if `listOpenIssues()` returns a truncated/empty page; no pagination, no guard) and Ralph never loops back to fix its own review; **#95** (`ralph/issue-69-dom-node-budgets`, human, stale base) is a `ralph/*`-branch PR sitting since morning with no gate status. So the guard trips on 2 → the 19 open `ralph-ready` issues never advance, and the skip was **silent** (Discord only pinged on non-skip runs). **Fixed on `claude/ralph-firing-issue-xqwpf9`:** the guard now classifies open Ralph PRs (failed/absent-gate = *wedged*, fresh pending = *healthy in-flight*) and fires a loud Discord alert naming the blocking PRs when wedged — single-flight still holds. **Immediate unblock is operational (Adrian's call, see Next):** clear #114 (address the review or close) and #95 (rebase or close).
