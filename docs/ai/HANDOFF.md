...
- **NEW: Issue #47 fleet-dispatch safety layer landed (2026-07-08)** — the
  build-first gate the mayor issue named before scheduling can happen.
  `scripts/fleet-dispatch.mjs` gained `applySafetyGates` (lease → overlap →
  budget) between tier-routing and dispatch: **B.2 claim/lease**
  (`lib/tasks/lease.mjs`, migration `0009_task_lease_touches.sql`, not yet
  applied) atomically claims a short-TTL lease immediately pre-dispatch,
  closing the double-dispatch race two concurrent runs could hit; **B.3
  file-overlap** (`lib/tasks/overlap.mjs` + revived `scripts/check-unit-
  overlap.mjs` CLI, registered in `docs/guardrails/registry.json` manual
  channel) refuses to fire a task whose declared `touches` overlap an
  in-flight task's; **B.1 spend ceiling** (`lib/tasks/budget.mjs` pure
  projection mined from `auto-assigner.mjs`'s `PRICE_PER_M_TOKENS` +
  `lib/ops/spend-log.mjs` committed ledger) caps per-run + daily PROJECTED
  USD spend (not metered actual). **`touches` isn't populated anywhere
  yet** — the overlap gate is an honor-system placeholder until a follow-up
  wires it. Migration 0009 not yet applied; new unit tests UNVERIFIED by a
  real test run (sandbox Bash approval unavailable) — see Next queue item 7
...
6. **clients repo cleanup** — tracked canonically as **hirobius/clients#10**
   ...
   can't diverge.
7. **Issue #47 fleet-dispatch safety layer landed (2026-07-08), verification
   + follow-ups open.** `applySafetyGates` (lease → overlap → budget) now
   runs inside `fleet-dispatch.mjs` before every dispatch — see the Now-section
   bullet + Done-log for the full shape. Before this ships live:
   - **Apply migration `0009_task_lease_touches.sql`** in Supabase (adds
     `lease_owner`/`lease_expires_at`/`touches` to `tasks`) — until then
     `fleet-dispatch.mjs`/`check-unit-overlap.mjs` exit cleanly naming the
     migration, same defensive pattern as 0008.
   - **Run `pnpm typecheck && pnpm test` for real** — this session's Bash
     tool calls beyond trivial reads required approval that wasn't grantable
     headless, so the new unit tests (lease/overlap/budget/spend-log +
     `applySafetyGates`) are UNVERIFIED by an actual run. Update this
     session's `--allowedTools` (or run locally) before trusting them green.
   - **Populate `touches` on tasks** — nothing writes it yet (no `/ops/tasks`
     UI toggle, no inference from the task body/diff), so the B.3 overlap
     gate is a no-op honor-system placeholder today. Needs its own follow-up
     issue: either an operator-editable field or inference from the dispatch
     issue body.
   - **B.4 (wall-clock/pod caps) + B.5 (Discord read commands)** — explicitly
     labeled "follow-ups" in issue #47's own DoD sequencing, not built.
   - **Wiring the actual scheduled mayor Routine** is still Adrian's call
     (needs a cadence decision) — the safety layer existing is the
     prerequisite the issue named, not the scheduling itself.
...
- 2026-07-08 (Issue #47 — fleet-dispatch safety layer, the build-first gate the mayor issue itself named before scheduling): scoped a large consolidated spec (mayor design + 5-item audit of deleted orchestration tooling) down to the three gates the issue's own DoD marks as blocking — B.1 spend ceiling, B.2 claim/lease, B.3 file-overlap — leaving B.4 (wall-clock/pod caps), B.5 (Discord read commands), and actually wiring the scheduled Routine as explicit follow-ups (the issue's own sequencing labels those "follow-ups"/Adrian's-cadence-call). Couldn't reach commit `662d94c^` (`git fetch --unshallow` needed approval unavailable headless) so reimplemented fresh from the issue's spec + `auto-assigner.mjs`'s still-live `PRICE_PER_M_TOKENS`/`costCeiling` idea rather than archaeology. New migration `0009_task_lease_touches.sql` (not yet applied) adds `lease_owner`/`lease_expires_at`/`touches` to `tasks`. Three new pure-logic modules, each independently unit-tested: `lib/tasks/lease.mjs` (`isLeased`/`filterUnleased` pure, caller-supplied `now`, throws if omitted — matches `fleet-watchdog.mjs`'s `findStale` convention; `claimLease` is an atomic conditional `UPDATE ... WHERE lease_expires_at IS NULL OR < now` so a lost race between two concurrent runs is a clean `claimed:false`, never a silent double-write; `releaseLease` clears it unconditionally on any terminal dispatch outcome), `lib/tasks/overlap.mjs` (`touchesOverlap`/`partitionByOverlap`/`findAllOverlaps` — plain path-prefix matching, no glob dependency since `picomatch` in package.json is a transitive pnpm-override pin, not importable; a task with no `touches` declared is a known gap and is never blocked nor blocks others, called out explicitly rather than silently claimed as complete), `lib/tasks/budget.mjs` (`projectedCostUsd`/`capByBudget` — narrows `auto-assigner.mjs`'s price table to the two models `tier.mjs` ever routes to, explicit module-doc caveat that this is a PROJECTED pre-dispatch estimate, not metered actual spend, since this repo never observes the real token usage of the `@claude` session it hands off to). New `lib/ops/spend-log.mjs` committed JSONL ledger, exact same shape/API as `run-log.mjs`/`notify.mjs` (`{path}` override, tolerant-of-bad-lines reader, `sumSpendSince` pure aggregator). `scripts/fleet-dispatch.mjs` gained `applySafetyGates(selected, {now, inFlightTasks, dailySpentUsd, ...})` — pure, wires lease→overlap→budget in that order — plus a live `fetchInFlightTasks` query, `--max-cost-usd`/`--daily-cost-usd`/`--lease-minutes` CLI flags, dry-run reporting of what each gate would block, and an `--apply` loop that claims the lease immediately pre-dispatch (releasing it on both success and failure paths) and appends the projected cost to the spend ledger on success. Revived `scripts/check-unit-overlap.mjs` as a read-only on-demand CLI (same `findAllOverlaps` the dispatcher's gate uses internally) and registered it in `docs/guardrails/registry.json` (`manual` firingChannel, `fixturePath: null` — same exemption shape as the existing Supabase-backed `audit-sites.mjs` entry, confirmed against `validate-fixture-proof-of-firing.mjs`'s explicit manual-channel exemption path rather than guessed). **Known gaps, documented rather than hidden:** `touches` is populated nowhere yet (no `/ops/tasks` UI toggle, no body/diff inference) so B.3 is an honor-system placeholder until a follow-up wires it; **this sandbox's Bash tool required approval for every command beyond trivial reads (`node --check`, `pnpm typecheck`, `vitest run` all blocked) so the new tests are UNVERIFIED by an actual run** — written to mirror existing test files' exact conventions (`tier.test.mjs`/`run-log.test.mjs`/`notify.test.mjs`/`fleet-dispatch.test.mjs`) but Adrian should run `pnpm typecheck && pnpm test` before merging, and update this session's `--allowedTools` if recurring. North-star flag surfaced once in the initial comment (infrastructure, not a shipped client-site feature) — proceeded since it's the issue's own explicit ask and the literal prerequisite for the mayor ever being schedulable.
