...
0. **Ralph — what's left for Adrian.** (a) **Label `ralph-approved` on PR
   #124** (the loop's first shipped PR — fixes #54; gate green + AI-approved,
   waiting on you). (b) Branch protection for `hds` + `site-engine`: required
   check `ralph-gate` (the plain status, NOT the `ralph-gate / ralph-gate`
   job) + "Allow auto-merge" — same two toggles done for ops. (c) Optional:
   `p0`–`p3` labels where ordering matters; `ralph-auto` on issues to
   batch-pre-approve merges. The old "ops Actions access" toggle is obsolete —
   the engine is public (`hirobius/ralph`).
