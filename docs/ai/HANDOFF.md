...
- **NEW: Issue #50 landed (2026-07-08)** — `/ops/tasks` now reflects live
  dispatch progress instead of sitting at `dispatched` until someone clicks
  Done. `lib/tasks/dispatch-status.mjs` + a new `getLinkedPullRequest` method
  on the GitHub port (`lib/github/issues.mjs`) resolve the PR linked to a
  task's `dispatch_url` and drive `dispatched → running → done/failed`; wired
  into `GET /api/tasks` (fail-soft, TTL-cached). **Needs migration
  `0009_task_pr_url.sql` applied** (paste-ready SQL in that file) — until then
  the status transitions still work (they use the existing 0008
  `dispatch_status` column) but `pr_url` writes silently no-op. Board also
  gets a dispatch-status badge, a "Recently completed" section, and the UX
  polish issue #50 asked for (grouped/titled Auto·Queue·Dispatch, confirm-on-
  Trash, inline action feedback, `aria-live`, mobile stacking).
...
- 2026-07-08 (Issue #50 — live dispatch-status feed + board UX polish, explicitly framed as a deferred build but dispatched directly via `/ops/tasks`, read as un-deferring it): **Backend:** new `getLinkedPullRequest({issueUrl})` method on the GitHub port (`lib/github/issues.mjs`) reads an issue's timeline (`GET .../issues/:n/timeline`) and picks the most recent `cross-referenced` event whose `source.issue.pull_request` is set (the same signal GitHub's own "Development" sidebar uses — no "Closes #" required, any mention works); pure `pickLatestLinkedPr(events)` exported for testing. New `lib/tasks/dispatch-status.mjs`: pure `deriveDispatchPatch(linkedPr, now?)` collapses onto the existing 5-value `dispatch_status` enum (migration 0008) rather than adding new states — merged→`done` (+ `status:'done'`, `completed_at`), closed-unmerged→`failed`, open→`running` — and `resolveLiveDispatchStatuses(tasks, {github, updateTask, sb})` applies it to every eligible row (`dispatch_url` set, `status!=='done'`, `dispatch_status` not already terminal — including legacy rows with a null `dispatch_status` from before 0008) in place + best-effort persists, TTL-cached per issue URL (60s, module-scope `Map`, same shape as `lib/projects/index.mjs`'s `attachRepoStatuses`) and fail-soft (a GitHub hiccup just leaves the row as-is, never throws). Wired into `GET /api/tasks` (`api/tasks.ts`) and the dev mirror (`scripts/tasks-middleware.mjs`) — no new `api/*.ts` file, stays 11/12. New migration `0009_task_pr_url.sql` (`pr_url text`, additive) — **NOT YET APPLIED**; until Adrian runs it, `pr_url` writes 42703 and get swallowed by the fail-soft persistence catch (dispatch_status/done detection still works since 0008's column already exists). `src/app/pages/ops/tasks/types.ts` gets `pr_url`. **Frontend (`TasksPage.tsx`):** dispatch-status badge (`DISPATCH_TONE`) + a "PR ↗" link per row; a "Recently completed" section (last 48h, independent of the status filter, since a just-finished task otherwise vanishes from the default 'open' view); Auto/Queue/Dispatch buttons grouped in a titled `role="group"` div (issue's "disambiguate the three similar toggles" ask) with explanatory `title` tooltips; Trash now `window.confirm`s; `act()` now reads the actual `/api/task-action` response (previously silently swallowed per the "surfaced on next poll" comment) and shows per-row inline feedback (`Badge`, success/danger tone, auto-clears after 5s) plus drives a visually-hidden `aria-live="polite"` region; mobile stacking for the dense action row via a scoped `<style>` media query (`MOBILE_CSS`, same pattern as `ClientReportPage.tsx`'s `PRINT_CSS` — inline styles can't express `@media`). Detail-drawer (issue's "optional" bullet) not built — smallest-version scope. **Tests:** `scripts/__tests__/dispatch-status.test.mjs` — `deriveDispatchPatch` (null/merged/closed-unmerged/open), `pickLatestLinkedPr` (no-PR, ignores plain-issue cross-refs, picks-latest-mapping-merged/draft/state), `resolveLiveDispatchStatuses` with a stub GitHub port (no-op without a port, skips no-url/terminal-status/legacy-null-status correctly, in-place patch + persistence on merge, fails soft when the port throws). **NOT VERIFIED BY TOOLING:** this session's sandbox blocked every `pnpm` invocation at the tool-approval layer (`pnpm --version` itself required approval with no interactive approver reachable, and no `node_modules`/global vitest/tsc were available as a workaround) — `typecheck`, `test`, `test:layout`, and `lint` could not be run. Every changed file was hand-reviewed for syntax/type correctness (types cross-checked against `Task`/`TaskAction`/`GitHubIssuePort`; `Badge` tone reuse over guessed CSS custom-property names after grepping the codebase for precedent) but this is a real gap — **Adrian should run the AUTO-VALIDATE suite on this branch before merging**, per CLAUDE.md.
