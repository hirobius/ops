...
| `StandingPage`, `ralphStatus.ts`, `lib/tasks/fleet*.mjs`, `lib/github/issues.mjs` | ops-dashboard (`0179nG1G`) | 2026-09-16 | **ACTIVE — full metadata + a baseline action set on every issue row, and surfacing sweep truncation. Branch `claude/ops-dashboard-open-issues-slp7tz`.**                                                                      |
