...
| `scripts/ralph-watchdog.mjs`, `lib/ops/ralph-watchdog.mjs`         | claude (backlog triage, local Windows session) | 2026-09-16 | **active — ralph-dispatch claim was stale (#350 merged). Branch `claude/watchdog-supervised-paths`: MERGE now enforces the ops#238 supervised-path boundary. LOCAL ONLY — not pushed; Adrian pushes.**                        |
