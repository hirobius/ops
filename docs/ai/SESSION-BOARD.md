...
| `scripts/ralph-watchdog.mjs`, `lib/ops/ralph-watchdog.mjs` (NEW)   | _(nobody)_                                          | —          | **RELEASED 2026-09-16 — built, 35 tests, verified in production (run #1, exit 0). Still unscheduled: the hourly caller is a `.github/workflows/*` file no agent token can push.**                                             |
| `docs/prospecting/`, `scripts/lib/query-presets.mjs`               | ralph-dispatch (`session_01ALKdCTLRLXykNrm4okMwfh`) | 2026-09-16 | **active — adding the high-ticket vertical tier (law/dental/medical/advisors/custom builders). Docs + preset data only; no scoring change.**                                                                                  |
