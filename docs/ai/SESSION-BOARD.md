...
| ---------------------------------------------------------------------------------------------- | ------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/ops/audit` (`FleetAuditPage`, `docs/ai/fleet-audit.json`), `SurfacesRail` tile, `routes.tsx` | claude (session_01EEogvt) | 2026-09-24 | **RELEASED same day — built and COMMITTED LOCALLY ONLY on `claude/hds-figma-pilot-verify-tz293h`; NOT pushed, per the current CLAUDE.md hard rule ("NEVER git push. Local commits only."), which no longer carries the remote-session `claude/*` carve-out it had earlier today. Adrian pushes or drops it. Adds a standing record of the 2026-09-24 fleet audit: 15 issues grouped by theme with what each one is FOR, plus a dated build-health snapshot. Deliberately does NOT render issue open/closed state — that is volatile, GitHub owns it, and asserting it here would manufacture the exact rot hds#285/hds#281/ops#417 are about. Did NOT touch `StandingPage.tsx` (held by session_01KaDkS8). Two gates fired and were honoured rather than bypassed: route-coverage (added `/ops/audit` to ALL_ROUTES) and dom-node-budgets (`routes.tsx` 26→27 for the one added route element; no other budget loosened).** |
| `lib/projects/freshness.mjs`, `lib/projects/index.mjs` (attach only)                           | claude (session_01EEogvt) | 2026-09-24 | **RELEASED same day — ops#417 built and pushed on `claude/hds-figma-pilot-verify-tz293h`. Scoped NARROWLY on purpose: adds `statusFreshness` to the ProjectStatus shape and derives it in `attachRepoStatuses`. Does NOT touch `StandingPage.tsx`, which consumes `/api/projects` and is held by session_01KaDkS8 since 2026-09-19 — rendering the new field is that holder's call. Why it exists: hds's `status.json` was 6 commits and ~3h behind on 2026-09-24 and `/ops` would have shown it as current. Note the commit time is fetched from GitHub rather than taken from `latestDeployment.createdAt` — a deploy timestamp under-reports for any repo that goes days between deploys, which ops itself does.**                                                                                                                                                                                                       |
| `scripts/audit-ralph-merge-boundary.mjs`, `scripts/ralph-supervised-paths.mjs`, `scripts/audit-stranded-branches.mjs`, `docs/secrets/`, `docs/ai/SURFACES.json`, `StandingPage.tsx` (freshness render only), decide_by sweep | claude (session_013YiCv3) | 2026-09-24 | **HELD — building ops#398/400/407/415/416/418/419 on worktree branches off `claude/open-issues-count-1i85qw`. StandingPage row above says HELD by session_01KaDkS8 since 2026-09-19 but later board notes say it is free; treating it as stale, touching only the freshness render.** |
