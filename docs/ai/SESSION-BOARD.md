...
| `scripts/check-migration-ledger.mjs`, its `registry.json` entry (ops#348)         | claude (ci)             | 2026-09-18 | **HELD — building the repo↔database migration-ledger gate. Touches `registry.json`, which PR #380 also edits — expect a one-line conflict. Does NOT touch `supabase/migrations/` (#387 adds `0015` there). Runs no production SQL: the apply/repair SQL is handed to Adrian on the issue.**                                                                     |
