...
| `docs/secrets/`, `scripts/check-secret-registry.mjs`, `scripts/check-secret-health.mjs`, two new `registry.json` rows (ops#415) | _(nobody)_                | —          | **RELEASED 2026-09-22 — built on `claude/secret-health`, committed locally, then PUSHED to `claude/secret-health` after Adrian granted remote sessions the same `claude/*` push carve-out `hds/CLAUDE.md` already had (2026-09-22). No PR opened — Adrian's call. 1776/1776 tests green, typecheck/lint/validate-guardrail-registry/check-validator-wiring all clean. See `docs/ai/DONE-LOG.md`'s 2026-09-22 entry for the RELEASE_PAT correction and the live-verified GitHub expiry header.** |
