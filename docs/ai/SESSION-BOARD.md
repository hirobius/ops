...
| `StandingPage`, `ralphStatus.ts`, `lib/tasks/fleet*.mjs`, `lib/github/issues.mjs` | _(nobody)_              | —          | **FREE — released 2026-09-16, session closed out. `listOpenIssues()` returns `{ issues, truncated, fetched }`, NOT a bare array — 3 callers updated. 5 commits pushed to `claude/ops-dashboard-open-issues-slp7tz`, UNMERGED, no PR.** |
