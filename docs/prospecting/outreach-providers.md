...
their deliverability model depends on _not_ letting cold email touch their IPs.
...
It's the best fit for _our_ situation specifically:
...
- **Agency workspace isolation** — if we later run outbound _per client_, Smartlead separates them cleanly.
...
1. **Push** — a script/endpoint reads _qualified, non-`do_not_contact`_ leads from
...
Cold email deliverability requires operational setup Smartlead automates _around_
but can't replace:
...
- **B2B cold email is legal in the US under CAN-SPAM** (we email _businesses_, not
...
configured in the Smartlead UI. The footer text itself is generated, not
hand-written: paste `node scripts/render-privacy-policy.mjs --footer`.
`webhookEventToPatch`'s `unsubscribed` case is
