...
**Remediation PRs in flight:** job-hunt [#23](https://github.com/hirobius/job-hunt/pull/23) (`.gitignore` → `.env*`) · access-tech [#16](https://github.com/hirobius/access-tech/pull/16) (add `.gitignore`) · clients [#28](https://github.com/hirobius/clients/pull/28) (`SECRETS.md` owner-split doc).
...
| `hirobius/job-hunt` | in-progress (near) | Internal | ✅ | ☐ | [#22](https://github.com/hirobius/job-hunt/issues/22) | `.gitignore` broadened to `.env*` in **PR [#23](https://github.com/hirobius/job-hunt/pull/23)**. Remaining: `.env.example` bare name, Bitwarden classification. `ANTHROPIC_API_KEY` only. |
...
| `hirobius/clients` | in-progress | multi-client ⚠️ | ✅ | ☐ | [#27](https://github.com/hirobius/clients/issues/27) | **Highest cross-contamination risk** (multi-tenant monorepo). Owner-split model documented in `SECRETS.md` (**PR [#28](https://github.com/hirobius/clients/pull/28)**). Clean today (all demo/preview apps). Pre-real-client: `apps/_template/.env.example` + move client keys out of `client.config.ts` literals. `.gitignore` ✅. |
...
| `hirobius/access-tech` | n/a (no secrets) | client | ✅ | ☐ | [#15](https://github.com/hirobius/access-tech/issues/15) | Static site, no secrets surface. `.gitignore` (with `.env*`) added preemptively in **PR [#16](https://github.com/hirobius/access-tech/pull/16)**. |
