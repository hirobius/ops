 ...
 */
export function renderArtifacts(config, opts = {}) {
  const valid = defineClient(config);
  return {
    slug: valid.slug,
    preset: valid.brand.palettePreset,
    configFile: configFileSource(valid),
    commands: deployCommands(valid, opts.photos ?? []),
  };
}

/**
 * Local photo paths the config actually references (hero + gallery).
 *
 * The download block is driven by the CONFIG, not the manifest: a download for
 * a photo the config never references is wasted bytes in the client repo, and a
 * referenced path with no download is a 404 on the live site.
 *
 * @param {object} valid a validated ClientConfig
 * @returns {Set<string>}
 */
function referencedPhotoPaths(valid) {
  const paths = [valid.hero?.image, ...(valid.gallery ?? []).map((g) => g.src)];
  return new Set(paths.filter((p) => typeof p === 'string' && p.startsWith('/')));
}

/**
 * `curl` block landing every referenced photo under the app's public/ dir
 * (ops#196). Pexels' image CDN needs no auth, so this is a plain download.
 *
 * @param {object} valid
 * @param {Array<{ url: string, path: string }>} photos the lead's photo manifest
 * @returns {string[]} lines, empty when nothing is referenced
 */
function photoDownloadBlock(valid, photos) {
  const referenced = referencedPhotoPaths(valid);
  const needed = photos.filter((p) => referenced.has(p?.path));
  if (!needed.length) return [];

  return [
    ``,
    `# stock imagery — swap when the client provides real photos (optional)`,
    `mkdir -p public/photos`,
    ...needed.map((p) => `curl -L ${JSON.stringify(p.url)} -o public${p.path}`),
  ];
}
...
function deployCommands(valid, photos = []) {
  const { slug } = valid;
  ...
  return [
    ...
    ...photoDownloadBlock(valid, photos),
    ``,
