 ...
 *
 * @deprecated — dead code, Duda retired 2026-06-30 (docs/ARCHITECTURE.md); scheduled for removal.
 *
 * Nothing in the UI reaches this module: ops#185 switched the Leads board's site
 * button from `'build'` to the working `'render'` action, so the only remaining
 * callers are `buildLeadSite` / `publishLeadSite` in lib/leads/pipeline.mjs, both
 * deprecated alongside this one. Do not build on it, do not wire the real Duda
 * Partner API described below, and do not extend the stub — the production render
 * target is the Astro clients factory (lib/render). Removal is tracked separately;
 * see the follow-up issue linked from ops#187.
 *
 * The stub is also a live fabrication-ban hazard while it exists: buildSite and
 * publishSite invent preview/editor/live URLs that never point at a real site.
 *
 ...
 * @property {string} external_site_id  the builder's (Duda) site identifier (idempotency key)
...
export async function buildSite(lead) {
  ...
  void content;
  const siteName =
    lead.external_site_id ||
    `stub-${slug(lead.name || lead.place_id || 'site')}-${String(lead.id || '').slice(0, 8)}`;
  return {
    external_site_id: siteName,
