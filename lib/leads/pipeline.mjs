...
/**
 * A `leads` row (snake_case, nullable columns) → the agent pipeline's lead input.
 *
 * The one mapping: production generation uses it, and so does the model eval's
 * `--from-db` mode (ops#7), so an eval scores exactly the input the agent gets.
 * Nulls become absent fields so LeadSchema's defaults apply.
 *
 * @param {Record<string, any>} row  a `leads` row
 * @param {unknown} [photos]  photos to send instead of the row's (e.g. stock fill)
 */
export function toAgentLead(row, photos = row.photos) {
  return {
    name: row.name,
    category: row.category ?? undefined, // let LeadSchema's default apply if null
    city: row.city,
    region: row.region,
...
export async function generateLeadSite(sb, leadId) {
  ...
  try {
    const result = await runPipeline(toAgentLead(lead, photos));
...
/**
 * Record the production URL for a lead once a human has done the prod deploy
 * (ops#44, rescoped 2026-09-16). Writes `live_url` ONLY — never lifecycle,
 * `published`, `preview_url`, `contacted_at`, or pitch fields. Publishing
 * stays a deliberate human action because it is the billing event; this only
 * removes the manual paste of the URL itself.
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 * @param {string} leadId
 * @param {{ liveUrl: string }} opts
 */
export async function recordLiveUrl(sb, leadId, { liveUrl } = {}) {
  const { data: lead, error: fetchError } = await getLead(sb, leadId);
  if (fetchError || !lead) return { status: 404, body: { error: 'lead not found' } };

  const { error: updateError } = await updateLead(sb, leadId, { live_url: liveUrl });
  if (updateError) return { status: 500, body: { error: updateError.message } };

  return { status: 200, body: { ok: true, liveUrl } };
}
