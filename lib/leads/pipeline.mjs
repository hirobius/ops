...
// DEPRECATED import — retired Duda path (ops#187). Consumed only by the
// deprecated buildLeadSite/publishLeadSite below; goes with them in the removal PR.
...
export async function generateLeadSite(sb, leadId) {
  ...
  try {
    const result = await runPipeline({
      ...
      photos: Array.isArray(lead.photos) ? lead.photos : undefined,
    ...
    return {
      status: 200,
      body: { ok: true, score: result.judge.overall, pass: result.judge.pass },
    };
  } catch (err) {
    await updateLead(sb, leadId, { status: 'sourced' });
 ...
 *
 * @deprecated — dead code, Duda retired 2026-06-30 (docs/ARCHITECTURE.md); scheduled for removal. No UI dispatches `'build'` since ops#185; the live hand-off is
 * `renderLeadSite` above. Kept only so the action dispatcher keeps compiling until
 * the removal PR.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 */
export async function buildLeadSite(sb, leadId) {
  ...
  try {
    ...
    const { error: updateError } = await updateLead(sb, leadId, {
      external_site_id: result.external_site_id,
 ...
 *
 * @deprecated — dead code, Duda retired 2026-06-30 (docs/ARCHITECTURE.md); scheduled for removal. No UI dispatches `'publish'` since ops#185. Publishing in the Astro
 * factory is a deliberate human deploy (it is the billing event), not this call.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 */
export async function publishLeadSite(sb, leadId) {
  const { data: lead, error: fetchError } = await getLead(sb, leadId);
  if (fetchError || !lead) return { status: 404, body: { error: 'lead not found' } };
  if (!lead.external_site_id) {
    return {
      status: 409,
      body: { error: 'no site to publish — build the site first', code: 'NO_SITE' },
    };
  }
  ...
  try {
    const result = await publishSite(lead.external_site_id);
    const { error: updateError } = await updateLead(sb, leadId, {
