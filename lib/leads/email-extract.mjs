...
/**
 * Localparts that are never a business contact, only site furniture. Matched
 * whole: substring-matching these would reject real addresses (`test` is inside
 * `protesting`, `name` inside `nameplate`).
 */
...
/**
 * Fragments that make a localpart automated wherever they appear in it. These
 * DO get substring-matched: a real fetch of registry.npmjs.org returned
 * `npm-oidc-no-reply@github.com`, which an exact-match list containing
 * `no-reply` waved straight through. Prefixes and suffixes are the norm, not
 * the exception — `bounce-noreply@`, `mailer-daemon@`, `noreply-alerts@`.
 */
const JUNK_LOCALPART_SUBSTRINGS = [
  'noreply',
  'no-reply',
  'no_reply',
  'donotreply',
...
export function isUsableAddress(address) {
  ...
  if (JUNK_LOCALPART_SUBSTRINGS.some((frag) => local.includes(frag))) return false;
