...
export function callEligibility(lead, now = new Date()) {
  ...
  //
  // `next_action_at` is #324's column (migration 0012_pitch_queue). The call
  // channel reuses it rather than carrying a second `callback_at`: one place to
  // write "come back Tuesday", one place to read it.
  if (l.next_action_at) {
