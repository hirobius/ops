...
- Choose a hero variant that suits the business, and order the sections by this trade's trust
  drivers — lead with whatever convinces this kind of buyer first, not a fixed template order.`;

/** Exported for the model eval report (ops#7), which redacts slugs out of error text. */
...
/**
 * A lead's photos, normalized to `{ src, alt }` and filtered to local paths.
 *
 * Accepts both shapes `lead.photos` can hold: plain strings (the #186 sourcing
 * shape) and the provenance objects ops#196 persists
 * (`{ url, path, alt, source, photographer }`). Stock entries carry their own
 * honest alt text — which is why it is preserved here rather than overwritten
 * with a "<business> photo" caption that would imply the client's own work.
 *
 * @param {{ name: string, photos?: Array<string|{path?: string, alt?: string}> }} lead
 * @returns {Array<{ src: string, alt: string }>}
 */
function localPhotos(lead) {
  return (lead.photos ?? [])
    .map((p) =>
      typeof p === 'string'
        ? { src: p, alt: `${lead.name} photo` }
        : { src: p?.path, alt: p?.alt || `${lead.name} photo` },
    )
    .filter((p) => typeof p.src === 'string' && p.src.startsWith('/'));
}

/**
 * Values assemble() substitutes when the lead lacks them — visible, never
 * invented. The phone is site-engine's fleet-standard placeholder
 * (`lead-to-config.ts` PLACEHOLDER_PHONE): legal under the schema's 10-digit
 * NANP rule, yet never dialable, because its exchange starts with 0. The old
 * `(000) 000-0000` fails that rule since the ops#310 re-sync.
 */
export const PLACEHOLDERS = { phone: '(555) 010-0000', email: 'owner@example.com' };
...
export function assemble(lead, content) {
  ...
  const [heroPhoto, ...galleryPhotos] = localPhotos(lead);
  return {
    slug: slugify(lead.name),
    business: {
      name: lead.name,
      phone: lead.phone ?? PLACEHOLDERS.phone,
      email: lead.email ?? PLACEHOLDERS.email,
    ...
    layout: {
      // `layout.variant` is deprecated in the schema (it only exists to map the
      // legacy 'B' onto the video hero) — no longer emitted, the default applies.
      // Both fields below fall through to the schema defaults when the model
      // omits them, so assemble() stays total for direct callers.
      ...(content.sectionOrder ? { sectionOrder: content.sectionOrder } : {}),
      ...(content.heroVariant ? { sections: { hero: { variant: content.heroVariant } } } : {}),
    },
    ...
    // First local photo is the hero, the rest are the gallery (ops#196).
    // Never remote URLs in either slot — defineClient's publicPath rejects
    // anything not under the app's public/ dir, so a remote entry would fail the
    // whole config rather than just that image. The hero carries the photo's own
    // alt: the schema requires imageAlt whenever image is set (ops#310).
    ...(heroPhoto ? { hero: { image: heroPhoto.src, imageAlt: heroPhoto.alt } } : {}),
 ...
 * @param {{ maxAttempts?: number, feedback?: string, model?: string, thinking?: object, onUsage?: Function }} [opts]
 *   `model` / `thinking` / `onUsage` are per-call overrides for the model eval (ops#7); the
 *   production pipeline passes none and runs on MODELS.strong.
...
export async function generate(lead, brief, opts = {}) {
  ...
  const model = opts.model ?? MODELS.strong;
