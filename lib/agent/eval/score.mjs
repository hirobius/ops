 ...
 *                bound of its 95% CI ≥ −tolerance (default 0, the issue's literal
 *                "overall ≥ current baseline")
 *   pass-rate    paired Δ judge pass rate, same test against −passTolerance (default 0).
 *                `pass` is what production regenerates on, so a higher mean with more
 *                failing sites has not held quality
 ...
 *                candidate's worst plausible pass rate (the pass-rate CI's lower bound)
 *
 * A check is true (holds), false (fails) or null (the CI straddles the margin — the data
 * can't tell yet). Verdict: too small a sample → INCONCLUSIVE; any false → KEEP; any
 * null → INCONCLUSIVE; else SWITCH.
 *
 * Why intervals: with raw means and tolerance 0, two equally good models split
 * ~53/47 SWITCH/KEEP across re-runs of the same command. The unit of the interval is
 * the LEAD (trials of one lead are averaged first) — trials share the lead, so they
 ...
 *   tolerance: non-inferiority margin on judged overall (1–5 scale);
 *   passTolerance: the same on pass rate (0–1, so 0.1 = 10 points)
 */
export function compareArms(
  result,
  baselineArm,
  candidateArm,
  { tolerance = 0, passTolerance = 0, minPairs = MIN_PAIRS, minLeads = MIN_LEADS } = {},
) {
  ...
  const passOk = nonInferior(pass, passTolerance);
  ...
  const checks = [
    ...
    },
    {
      ...
      detail:
        ...
        `margin −${tolerance} → ${decided(qualityOk)} · means ${fmt(candidateMeanOverall)} vs baseline ${fmt(baselineMeanOverall)}`,
    },
    {
      name: 'pass-rate',
      ok: passOk,
      detail:
        `judge pass rate ${fmt(candidatePassRate * 100, 0)}% vs baseline ${fmt(baselinePassRate * 100, 0)}%; ` +
        `Δ ${pts(pass.mean)} pts, 95% CI [${pts(pass.low)}, ${pts(pass.high)}] pts; margin −${passTolerance * 100} pts → ${decided(passOk)}`,
    },
    {
      name: 'reliability',
