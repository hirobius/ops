 ...
 * @param {{ tolerance?: number, passTolerance?: number }} [opts]
 * @returns {string}
 */
export function renderMarkdown(result, { tolerance = 0, passTolerance = 0 } = {}) {
  ...
  out.push(
    `Run ${result.startedAt} · leads: ${o.source ?? 'n/a'} (${result.leads.length}) · trials: ${o.trials} · ` +
      ...
      `margins: overall −${tolerance}, pass rate −${passTolerance * 100} pts`,
  );
  if (result.pricing) {
  ...
  for (const c of comparisons) {
    ...
    if (c.verdict === 'INCONCLUSIVE' && c.checks.some((check) => check.ok === null)) {
      out.push(
        '',
        '> `[?]` = the 95% interval straddles the margin, so the data can’t call it yet. More leads narrow the interval, ' +
          'but at margin 0 two equally good models stay INCONCLUSIVE however many leads run. The margin you will accept ' +
          '(`--tolerance`, `--pass-tolerance`) is the decision to make.',
      );
    }
    out.push('');
  }
  ...
  out.push(
    ...
    `- **generate → SWITCH** needs all of: ≥ ${MIN_PAIRS} paired samples over ≥ ${MIN_LEADS} leads; ` +
      `the 95% CI of Δ mean judged overall (candidate − baseline, one Δ per lead) has its lower bound ≥ −${tolerance}; ` +
      `the same for Δ judge pass rate against −${passTolerance * 100} pts; generation failure rate no higher than baseline; ` +
    ...
    '- **KEEP** when any check fails outright — for quality and pass rate, the whole interval sits below the margin. ' +
      '**INCONCLUSIVE** when the sample is too small, or no check fails but an interval straddles its margin.',
