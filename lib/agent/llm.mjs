 ...
 * @param {object} [opts.thinking]  e.g. `{ type: 'disabled' }`; omitted → the model's default
 *   (Opus 4.8 runs without thinking, Sonnet 5 / Opus 5 run adaptive). Production passes none.
 * @param {(call: { model: string, usage: object, stopReason: string }) => void} [opts.onUsage]
 *   observer for the real token usage of this call — the model eval (ops#7) prices from it.
...
export async function callStructuredTool({
  ...
  // Report before inspecting the content: a call that returns no tool use was
  // still billed, and an eval that dropped it would under-count a model's cost.
  onUsage?.({ model, usage: response.usage, stopReason: response.stop_reason });
