...
/**
 * The combined pre-render gate `generate()` runs on the agent's palette.
 *
 * Returns a `note` written for the repair loop — it names the preset that was
 * echoed and every pair that failed, with measured ratios, so one regeneration
 * can fix everything rather than trading one problem per round-trip.
 *
 * An absent palette passes: overrides are optional at assemble time, and a
 * config without them is valid (it falls back to the preset). Requiring one is
 * the forced-tool schema's job, not this function's.
 *
 * A malformed hex fails loudly rather than being scored — contrastRatio throws,
 * and swallowing that would let a broken palette through the one check meant to
 * catch it.
 *
...
export function checkPalette(palette) {
  if (!palette) return { ok: true, note: '' };

  const problems = [];

  const lazy = isLazyPalette(palette);
  if (lazy) {
    problems.push(
      `- palette: you emitted the stock '${lazy}' preset verbatim. Derive a distinctive ` +
        `palette from this business's trade and locale instead — stock preset values are a failure.`,
    );
  }

  try {
    for (const f of checkContrast(palette).failures) {
      problems.push(
        `- palette contrast: ${f.pair} is ${f.ratio.toFixed(2)}:1, below the WCAG AA minimum of ` +
          `${f.min}:1. The site build gate will reject this. Darken or lighten one of the pair.`,
      );
    }
  } catch (err) {
    problems.push(`- palette: ${err instanceof Error ? err.message : String(err)}`);
  }

  return { ok: problems.length === 0, note: problems.join('\n') };
}
