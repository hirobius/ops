...
/**
 * Keys of every live (non-deleted, not-yet-done) `github:*`-sourced task —
 * the "stored" side of the importer's reconcile diff (ops#99,
 * lib/tasks/reconcile-github-tasks.mjs). Scoped to `github:%` so the reconcile
 * never touches client/lead-pipeline tasks from other sources.
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 */
...
/**
 * Key + tags of every live `github:*`-sourced task, as stored BEFORE an
 * import's upsert overwrites them — the "before" side of the importer's
 * needs-adrian pager diff (ops#51, lib/tasks/needs-adrian-pager.mjs). Same
 * scope as `listGithubTaskKeys`; a separate query rather than widening that
 * one's `select('key')` so its existing callers/shape stay unchanged.
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 */
export function listGithubTaskTags(sb) {
  return sb
    .from('tasks')
    .select('key, tags')
    .like('source', 'github:%')
...
/**
 * Retires tasks by key — flips `status` to `'done'`, not a delete. Used by the
 * importer's reconcile step for `github:*` keys that dropped out of the live
 * open-issues set (closed, or a renamed/transferred repo's old slug). Self-heals:
 * a false flip from a degenerate-but-plausible feed corrects itself on the next
 * complete import, since `mapIssuesToTasks` always re-upserts `status: 'open'`.
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 * @param {string[]} keys
 */
export function retireTasks(sb, keys) {
  if (!Array.isArray(keys) || keys.length === 0) return { data: [], error: null };
  return sb.from('tasks').update({ status: 'done' }).in('key', keys);
}
