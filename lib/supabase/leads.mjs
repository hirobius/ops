 ...
 * re-scrape can't resurrect or overwrite an opt-out, or write a deleted
 * business's details back onto the minimal record a deletion request leaves
 * (lib/compliance/privacy-request.mjs, and the privacy policy's promise).
 *
 * The lookup fails CLOSED: any error returns it and upserts nothing. The one
 * exception is the `do_not_contact` column not existing yet (migration 0007 not
 * applied), where there can be no suppressed rows to protect, so ingest proceeds
 * with a plain upsert and the guard activates once 0007 lands.
 */
export async function upsertLeads(sb, rows) {
  ...
  if (placeIds.length) {
    ...
    if (supErr && !isMissingDoNotContactColumn(supErr)) return { data: null, error: supErr };
...
/** Pre-0007 only: PostgREST reports the absent column as Postgres 42703 "column … does not exist". */
function isMissingDoNotContactColumn(error) {
  const message = String(error?.message ?? '');
  return (
    /do_not_contact/.test(message) && (error?.code === '42703' || /does not exist/i.test(message))
  );
}
 ...
 */
/**
 * The stored `preview_url` values, so stage 5's liveness can be probed.
 *
 * A sibling of `leadFunnel` rather than part of it: the funnel is nine cheap
 * COUNT queries and must stay that way, while this one returns rows and feeds
 * network probes (ops#322). Capped, because a poll must never turn into an
 * unbounded fan-out.
 */
export async function publishedUrls(sb, limit = 200) {
  ...
  return { data: (data ?? []).map((r) => r.preview_url).filter(Boolean), error: null };
}
