...
export async function getServiceClient() {
  const rawUrl = firstEnv('SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_URL', 'SUPABASE_PUBLIC_URL');
  const key = firstEnv('SUPABASE_SERVICE_ROLE_KEY', 'SUPABASE_SERVICE_KEY')?.trim();

  // Sanitize the URL: env values pasted from a dashboard often carry a trailing
  // slash, surrounding whitespace, or the full /rest/v1 endpoint — any of which
  // makes supabase-js build a bad path ("Invalid path specified in request URL").
  const url = rawUrl
    ?.trim()
    .replace(/\/+$/, '')
    .replace(/\/rest\/v1\/?$/, '');
