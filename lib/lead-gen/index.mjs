...
function mapPlace(p, { niche, city, region, metro }) {
  ...
  const place_id =
    p.place_id ||
    p.google_id ||
    `outscraper-${slug(p.name || niche)}-${slug(p.full_address || metro)}`;
  ...
  return {
    ...
    google_maps_url: place_id ? `https://www.google.com/maps/place/?q=place_id:${place_id}` : null,
...
function mockLeads({ niche, metro, city, region, count }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    leads.push({
      ...
      // 10-digit NANP with the unassigned 555 area code: passes the ClientConfig
      // phone rule (ops#310) yet is never dialable — see tests/lead-gen.
