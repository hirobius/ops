 ...
 * /ops/tasks manual "Dispatch" button, or scripts/fleet-dispatch.mjs). This
 * is the ONE seam for lifecycle writes (ops#139): `scripts/fleet-dispatch.mjs`
 * and `scripts/fleet-watchdog.mjs` route every dispatch-lifecycle mutation
 * through here (`dispatch`/`redispatch`/`flag`) rather than writing
 * `dispatch_status`/`claimed_by`/etc. directly via `setTaskFields` — each
 * lifecycle field has exactly one writer. `redispatch` (Slice 5) re-kicks an
 * already-dispatched task the watchdog found stale; `flag` blocks one that's
...
const SIMPLE = {
  ...
  // Stale-dispatch watchdog (epic #41 Slice 5, ops#139): a dispatch that's
  // exhausted its re-dispatch retries gets flagged for a human instead of
  // re-kicked forever.
 ...
 * @param {{ key: string, action: string, actor?: string, comment?: boolean, maxRetries?: number }} input
 * @param {{ github?: import('../github/issues.mjs').GitHubIssuePort | null }} [deps]
 * @returns {Promise<{ status: number, body: object }>}
 */
export async function applyTaskAction(
  sb,
  { key, action, actor, comment, maxRetries },
  { github } = {},
) {
  ...
  if (action === 'redispatch') return redispatch(sb, key, { comment, maxRetries, github });
 ...
 *
 * Stamps `last_dispatched_at`/`dispatch_count` too (ops#139) — this is the
 * ONE seam for dispatch-lifecycle writes, so every dispatch source (the
 * /ops/tasks manual button, /admin/approvals, `scripts/fleet-dispatch.mjs`)
 * leaves a consistent trail the Slice 5 watchdog can judge staleness from,
 * not just the ones that happened to pre-stamp it themselves.
 *
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 */
...
/**
 * Re-kicks an already-dispatched task that the stale-dispatch watchdog
 * (`scripts/fleet-watchdog.mjs`, epic #41 Slice 5) found has gone quiet:
 * bumps `dispatch_count`, restamps `last_dispatched_at`, and re-asserts
 * `dispatch_status='dispatched'` — the same three fields the watchdog used
 * to write directly via `setTaskFields` before this seam existed (ops#139).
 *
 * Leaving a fresh `@claude` comment is opt-in (`comment: true`, the
 * watchdog's `--comment` flag) and best-effort: a missing port, a missing
 * `dispatch_url`, or a thrown comment call all fall back to a `commentError`
 * note on the response rather than failing the whole action — the field
 * restamp (the part that actually un-stalls the watchdog's next pass) still
 * lands either way.
 *
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 */
async function redispatch(sb, key, { comment, maxRetries, github }) {
  const { data: task, error: fetchError } = await getTask(sb, key);
  if (fetchError || !task) return { status: 404, body: { error: 'task not found' } };
  ...
  if (comment) {
    if (!github) {
      commentError = 'GITHUB_TOKEN not set — comment skipped (fields still re-dispatched).';
    } else if (!task.dispatch_url) {
      commentError = 'no dispatch_url on record — comment skipped.';
    } else {
      try {
        await github.commentOnIssue({
          ...
          body:
            ...
            `re-kicking, retry ${nextCount}/${maxRetries}.`,
        });
      } catch (err) {
        commentError = messageOf(err);
      }
    }
  }

  return { status: 200, body: { ok: true, ...patch, ...(commentError ? { commentError } : {}) } };
}
