...
import { BLOCKING_LABELS } from './fleet.mjs';
 ...
 * guard, so this genuinely jumps the queue — that is the point of the button,
 * and the reason it is separate from "Queue" rather than folded into it.
 *
 * Needs "Actions: write", a DIFFERENT scope from the "Issues: write" every
 * other action here uses, so the failure names that scope specifically — a
 * token that labels fine and dispatches 404 is otherwise a baffling error.
 *
 * @param {string} key `github:<owner>/<repo>#<n>`
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 */
