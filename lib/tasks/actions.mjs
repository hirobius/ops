 ...
 * guard AND steals any claim (ralph.yml header), so this genuinely jumps the
 * queue — that is the point of the button, and the reason it is separate from
 * "Queue" rather than folded into it. It is also why the issue is READ first:
 * the row the operator tapped is up to a poll old, and a dispatch is only safe
 * for an issue that is open, in the queue lane right now (`ralph-ready`, not
 * parked, not human-gated — `laneOf`, the selector's own rule) and not already
 * held by an iteration (`ralph-wip`). Anything else is a 409 and nothing fires.
 *
 ...
 *
 * @param {{ state?: string, labels?: string[] }} issue a fresh `getIssue` read
