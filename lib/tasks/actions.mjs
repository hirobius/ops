 ...
 *
 * `dispatch` stamps `dispatch_status: 'dispatched'` — whichever caller reaches
 * it (the /ops/tasks manual "Dispatch" button, or scripts/fleet-dispatch.mjs,
 * which already stamps 'dispatched' itself via setTaskFields before calling
 * this).
 */
...
const SIMPLE = {
  ...
  // (migration 0008) — scripts/fleet-dispatch.mjs only picks up auto_ok=true
  // rows. No UI surfaces this toggle (ops#157); the actions stay for the
  // script.
 ...
 * @param {{ key: string, action: string, actor?: string, priority?: string|null }} input
 * @param {{ github?: import('../github/issues.mjs').GitHubIssuePort | null }} [deps]
 * @returns {Promise<{ status: number, body: object }>}
 */
export async function applyTaskAction(sb, { key, action, actor, priority }, { github } = {}) {
