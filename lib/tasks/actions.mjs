 ...
 *
 * `dispatch` is the ONE writer of the dispatch-lifecycle fields
 * (`dispatch_status`, `last_dispatched_at`, `dispatch_count`) — every
 * dispatch source (the /ops/tasks manual "Dispatch" button,
 * scripts/fleet-dispatch.mjs) routes through it rather than pre-stamping
 * those fields itself, so the staleness watchdog sees a consistent trail
 * regardless of where a dispatch came from. `redispatch`/`flag` are this
 * same seam's other two lifecycle writes, used by scripts/fleet-watchdog.mjs
 * instead of a second, divergent mutation path (ops#139).
 */

import { getTask, updateTask } from '../supabase/tasks.mjs';
import { BLOCKING_LABELS, blockingLabelOf, laneOf } from './fleet.mjs';
...
/**
 * The fleet-repo workflow file the "Run Ralph" board action dispatches
 * (ops#113). Exported so other dispatch entry points (scripts/discord-bot.mjs's
 * `!dispatch`, ops#256) fire the same workflow file instead of hardcoding a
 * second copy of the string.
 */
export const RALPH_WORKFLOW_FILE = 'ralph.yml';
...
const SIMPLE = {
  ...
  // Stale-dispatch watchdog's exhausted-retries outcome (ops#139): matches
  // what scripts/fleet-watchdog.mjs's `flag` used to write directly.
...
export async function applyTaskAction(
  ...
  // Mirror-free: /ops/standing acts on live GitHub issues, including repos
  // the importer has never seen.
  if (action === 'queue_on') return labelIssueDirect(key, github, RALPH_READY_LABEL, true);
  if (action === 'queue_off') return labelIssueDirect(key, github, RALPH_READY_LABEL, false);
  if (action === 'unblock') return unblockIssue(key, github);
  if (action === 'bump_priority') return setPriorityDirect(key, github, priority);
  if (action === 'auto_on_direct') return labelIssueDirect(key, github, RALPH_AUTO_LABEL, true);
  if (action === 'auto_off_direct') return labelIssueDirect(key, github, RALPH_AUTO_LABEL, false);
  if (action === 'park_direct') return labelIssueDirect(key, github, RALPH_PARKED_LABEL, true);
  if (action === 'run_now') return runNowDirect(key, github);

  return { status: 400, body: { error: `unknown action: ${action}` } };
}

/** The dispatch_count a dispatch/redispatch write should stamp — missing/non-numeric reads as 0. */
...
/**
 * Add or remove one label on a GitHub issue, addressed ONLY by its
 * `github:<owner>/<repo>#<n>` key — no Supabase row required.
 *
 * This is the seam `/ops/standing` acts through, and the distinction is
 * load-bearing rather than stylistic. `toggleIssueLabel` reads the mirror first
 * and 404s when the row is absent; Standing lists issues straight from GitHub's
 * identity feed, which spans repos the importer has never touched, so a
 * mirror-backed action would fail on exactly the issues Standing newly
 * surfaced. It also means the mirror can eventually be retired without taking
 * the actions with it.
 *
 * Labels ARE the loop's state store, so writing to GitHub is writing to the
 * real thing; the mirror was only ever a cache of it.
 *
 * @param {string} key `github:<owner>/<repo>#<n>`
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 * @param {string} label
 * @param {boolean} on
 */
async function labelIssueDirect(key, github, label, on) {
  if (!github) {
    return {
      status: 503,
      body: {
        error:
          'GITHUB_TOKEN not set — needed to update the issue label. Add it in Vercel → ' +
  ...
  return { status: 200, body: { ok: true, label, on } };
}

/**
 * Standing's "Unblock": drop every human-gate label, then queue it.
 *
 * The inverse of the gesture that put it there, and mirror-free for the same
 * reason `labelIssueDirect` is. Removing ALL of BLOCKING_LABELS rather than the
 * one the badge happened to show matters: an issue can carry `needs-adrian` AND
 * `blocked` (ops#238 and ops#9 both do), and clearing only the displayed one
 * leaves it in the same lane — which, from the operator's seat, is another
 * button that "does nothing".
 *
 * Removal is idempotent (`removeLabel` treats 404 as success), so a label that
 * was not there is not an error.
 *
 * @param {string} key `github:<owner>/<repo>#<n>`
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 */
async function unblockIssue(key, github) {
  if (!github) {
    return {
      status: 503,
      body: {
  ...
  try {
    for (const label of BLOCKING_LABELS) {
      await github.removeLabel({ issueUrl, label });
    }
  ...
  return { status: 200, body: { ok: true, unblocked: [...BLOCKING_LABELS] } };
}

/**
 * Set exactly one of p0–p3 on a GitHub issue, addressed only by its key.
 *
 * The mirror-free twin of `setPriority`. Priority labels are mutually
 * exclusive, so every other one comes off first — otherwise a bump to p0 leaves
 * p2 in place and `ralph/next.sh` keeps ranking it as p2, which would make the
 * tap look like it worked while changing nothing about what the loop does.
 *
 * @param {string} key `github:<owner>/<repo>#<n>`
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 * @param {string|null|undefined} priority one of p0–p3, or null to clear
 */
async function setPriorityDirect(key, github, priority) {
  if (!github) {
    return {
      status: 503,
      body: {
  ...
  try {
    ...
    if (priority) await github.addLabel({ issueUrl, label: priority });
  } catch (err) {
    return githubPortError(err, { action: 'label update', code: 'GITHUB_LABEL_FAILED' });
  }

  return { status: 200, body: { ok: true, priority: priority ?? null } };
}

/**
 * Standing's "Run" — fire `ralph.yml` at one issue, from its key alone.
 *
 * The mirror-free twin of `ralph_dispatch`, which resolves a Supabase row
 * first and so 404s on exactly the repos Standing newly surfaced.
 *
 * An explicit issue number skips `ralph/next.sh` altogether AND steals any
 * claim (ralph.yml header; the reusable's guard only re-checks open +
 * `ralph-ready`), so this genuinely jumps the queue — that is the point of the
 * button, and the reason it is separate from "Queue" rather than folded into
 * it. It is also why the issue is READ first: the row the operator tapped is
 * up to a poll old, and a dispatch is only safe for an issue that is open, in
 * the queue lane right now (`ralph-ready`, not parked, not human-gated —
 * `laneOf`, the selector's own rule), not already held by an iteration
 * (`ralph-wip`), and carrying the DoD marker next.sh would otherwise park it
 * for on sight. Anything else is a 409 and nothing fires.
 *
 * What stays skipped, by design of the override: single-flight (a second Ralph
 * PR can stack on one already open), the attempt and lifetime budgets, and the
 * merged-PR-newer-than-ralph-ready guard. The row's confirm names those.
 *
 * Needs "Actions: write", a DIFFERENT scope from the "Issues: write" every
 * other action here uses, so a refused dispatch names that scope specifically.
 * A dispatch 404 is NOT a scope problem here: the issue read just succeeded
 * with the same token, so the repo is visible and the workflow file is what is
 * missing — sending the operator to rotate a working token is the wrong fix.
 *
 * @param {string} key `github:<owner>/<repo>#<n>`
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 */
async function runNowDirect(key, github) {
  if (!github) {
    return {
      status: 503,
      body: {
  ...
  let issue;
  try {
    issue = await github.getIssue({ owner: ref.owner, repo: ref.repo, number: ref.number });
  } catch (err) {
    return githubPortError(err, {
      action: 'issue read',
  ...
  const refusal = runRefusal(issue, name);
  ...
  try {
    ...
    if (/** @type {{ status?: number }} */ (err)?.status === 404) {
      return {
        ...
        body: {
          error:
            `no ${RALPH_WORKFLOW_FILE} in ${ref.owner}/${ref.repo} — GitHub answered 404 to the ` +
            'workflow dispatch, so there is no loop in that repo to run. Queue it instead, or add ' +
            `the ${RALPH_WORKFLOW_FILE} caller to that repo.`,
          code: 'RALPH_WORKFLOW_MISSING',
        },
      };
    }
    return githubPortError(err, {
...
/**
 * Why `run` must not fire at this issue, or null when it may.
 *
 * @param {{ state?: string, labels?: string[], hasDod?: boolean }} issue a fresh `getIssue` read
 * @param {string} name `owner/repo#n`, for the message
 * @returns {{ error: string, code: string } | null}
 */
function runRefusal(issue, name) {
  const labels = Array.isArray(issue?.labels) ? issue.labels : [];
  if (issue?.state !== 'open') {
    return { error: `${name} is closed — there is nothing to run.`, code: 'RUN_ISSUE_CLOSED' };
  }
  const lane = laneOf({ labels });
  if (lane === 'parked') {
    return {
      error: `${name} is ${RALPH_PARKED_LABEL} — Re-queue it first; run only jumps the queue for an issue already in it.`,
      code: 'RUN_NOT_QUEUED',
    };
  }
  if (lane === 'blocked') {
    return {
      error: `${name} carries ${blockingLabelOf({ labels })} — Unblock it first; run only jumps the queue for an issue already in it.`,
      code: 'RUN_NOT_QUEUED',
    };
  }
  if (lane !== 'queue') {
    return {
      error: `${name} is not queued — add ${RALPH_READY_LABEL} first; run only jumps the queue for an issue already in it.`,
      code: 'RUN_NOT_QUEUED',
    };
  }
  if (labels.includes('ralph-wip')) {
    return {
      error: `${name} already carries ralph-wip — an iteration holds it, and a dispatch naming it would steal that claim.`,
      code: 'RUN_ALREADY_WIP',
    };
  }
  // `!== true`, not falsy-on-false: a verdict the port could not give fails
  // closed, as next.sh does on an API failure.
  if (issue?.hasDod !== true) {
    return {
      error:
        `${name} has no \`- [ ]\` checklist or acceptance/DoD section — ralph/next.sh parks that ` +
        'on sight, and a dispatch skips next.sh, so it would spend a whole iteration instead. ' +
        'Add a DoD to the issue body first.',
      code: 'RUN_NO_DOD',
    };
  }
  return null;
...
/**
 * The stale-dispatch watchdog's re-kick (ops#139, extends Slice 5): bumps
 * `dispatch_count`, restamps `last_dispatched_at`, and re-asserts
 * `dispatch_status: 'dispatched'` — the same fields `dispatch` stamps, so a
 * re-dispatched task looks identical to a freshly-dispatched one to anything
 * reading the board.
 *
 * The DB write is the critical path; the `comment` re-ping is best-effort and
 * never fails the action — `commentNote` in the response tells the caller
 * (scripts/fleet-watchdog.mjs) whether/why it posted, so it can log the same
 * detail its old inline implementation did.
 *
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 * @param {{ comment?: boolean, maxRetries?: number }} opts
 */
async function redispatch(sb, key, github, { comment, maxRetries } = {}) {
  ...
  const { error: updateError } = await updateTask(sb, key, patch);
  if (updateError) return { status: 500, body: { error: updateError.message } };

  let commentNote;
  if (comment) {
    if (!github) {
      commentNote = 'skipped: GITHUB_TOKEN not set';
    } else if (!task.dispatch_url) {
      commentNote = 'skipped: no dispatch_url on record';
    } else {
      const retryLabel = Number.isFinite(maxRetries)
        ? `${patch.dispatch_count}/${maxRetries}`
        : `#${patch.dispatch_count}`;
      try {
        await github.commentOnIssue({
          issueUrl: task.dispatch_url,
          body: `@claude this dispatch went stale (no PR after the staleness window) — re-kicking, retry ${retryLabel}.`,
        });
        commentNote = 'posted';
      } catch (err) {
        commentNote = `failed: ${messageOf(err)}`;
      }
    }
  }

  return { status: 200, body: { ok: true, ...patch, commentNote } };
}
