...
export async function applyTaskAction(sb, { key, action, actor }, { github } = {}) {
  ...
  if (action === 'ralph_dispatch') return ralphDispatch(key, github);
 ...
 * genuinely jumps the queue. Reads owner/repo/#n straight off the task
 * `key` (`github:<owner>/<repo>#<n>`) — no Supabase round trip, nothing on
 * the row changes, so unlike `dispatch`/`ralph_ready_*`/`ralph_approve` this
 * doesn't need `sb`.
 *
 ...
 * override.
 */
async function ralphDispatch(key, github) {
  if (!github) {
    return {
      status: 503,
      body: {
    ...
    return {
      ...
      body: {
        error:
          ...
          `or missing the "${permission}" permission — rotate it in Vercel → ` +
