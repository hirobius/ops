...
/** The label a parked issue carries until a human clears it (ops#141). */
...
export async function applyTaskAction(sb, { key, action, actor }, { github } = {}) {
  ...
  if (action === 'ralph_requeue') return requeueParked(sb, key, github);
...
/**
 * The parked-inbox one-tap re-queue (ops#141): removes `ralph-parked` and
 * adds `ralph-ready` on the linked GitHub issue — exactly the documented
 * manual-recovery gesture (`ralph-parked`'s own park comment says "re-add
 * ralph-ready"), which per ops#121 also resets the attempt budget
 * (`count_failed_attempts` only counts failures newer than the latest
 * `ralph-ready` labeled event). Idempotent: re-queuing an already-queued
 * issue just re-applies the same two label ops and returns 200.
 *
 * Deliberately does NOT touch `needs-adrian` — the panel only offers this
 * gesture for `ralph-parked` rows; a `needs-adrian` row gets a hint instead
 * (see ralph-parked.mjs's hasDodMarker), since blindly re-queuing one whose
 * body still has no DoD marker just bounces it straight back via the intake
 * filter.
 *
 * @param {import('../github/issues.mjs').GitHubIssuePort | null | undefined} github
 */
async function requeueParked(sb, key, github) {
  if (!github) {
    return {
      status: 503,
      body: {
  ...
  if (!issueUrl) {
    return {
      status: 400,
      body: { error: 'task has no linked GitHub issue — cannot re-queue.' },
    };
  }
  ...
  try {
    ...
    return githubPortError(err, { action: 're-queue', code: 'GITHUB_LABEL_FAILED' });
  }

  const tags = Array.isArray(task.tags) ? task.tags : [];
  const nextTags = [...tags.filter((t) => t !== RALPH_PARKED_LABEL)];
  if (!nextTags.includes(RALPH_READY_LABEL)) nextTags.push(RALPH_READY_LABEL);
