...
/**
 * The repo set is CONFIGURED (ops#405): `listOpenIssues()` fetches GitHub's
 * authenticated-identity feed — every open issue the token can see, across
 * every owner — but filters it down to `lib/tasks/fleet.mjs`'s `FLEET_REPOS`
 * before this function ever sees a row, so a repo joins this view by being
 * added there, not merely by existing. (Before ops#405, it joined by
 * existing, and that is why 114 of 209 open issues on 2026-09-19 came from
 * repos that were never the fleet.) The sibling `?ralph=1` read fans out over
 * its own, deliberately narrower `FLEET_REPOS` in `api/tasks.ts` — Ralph-loop
 * dispatch targets, not every fleet repo — and cannot see a new repo until
 * someone edits it.
 *
...
export async function buildFleetStatus(gh, ctx = {}) {
  ...
  const { blocked, queue, backlog, sev1, decisions, repos, total } = sortFleetLanes(issues);
  const owners = ownersOf(repos);

  const errors = [];
  let prs = [];
  try {
    // The search is scoped by owner, so it also returns PRs from non-fleet
    // repos under the same account; keep only the fleet's (ops#405).
    prs = filterFleetIssues(await gh.searchOpenPrs({ query: openPrSearchQuery(owners) }));
  } catch (err) {
  ...
  return {
    ...
    body: {
      ...
      sev1,
      // ops#418: every open decision-template issue, nearest decide_by first.
      decisions,
      total,
      prs,
      loop,
