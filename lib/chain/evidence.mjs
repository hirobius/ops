 ...
 * @param {{ numbers: number[], complete: boolean } | null} [input.openIssues]
 *   open hirobius/ops issue numbers from the fleet read; `complete` false when
 *   the read was truncated
 */
export function deriveChain({ funnel = {}, env = {}, liveness = null, openIssues = null } = {}) {
  // Open/closed is only knowable from a COMPLETE read of open issues. A partial
  // or missing read says "unknown" (null) — never "closed", which it cannot prove.
  const openSet = openIssues?.complete ? new Set(openIssues.numbers) : null;
  const links = STAGES.map((stage) => {
    const count = funnel[stage.metric] ?? null;
    const missingEnv = stage.envKeys.filter((k) => env[k] !== true);
    const missingOptional = (stage.optionalEnvKeys ?? []).filter((k) => env[k] !== true);
    const issueStates = stage.issues.map((n) => ({ n, open: openSet ? openSet.has(n) : null }));
    ...
    return {
      ...
      issueStates,
      state,
      ...(stage.metric === 'published' ? { liveness } : {}),
    };
  });
