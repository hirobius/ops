...
/** Which rule fired, e.g. `denylist entry 3 (PII_DENYLIST)` or `phone: US phone number`. */
function ruleLabel(finding) {
  return finding.rule === 'denylist' ? finding.detail : `${finding.rule}: ${finding.detail}`;
}
...
function describeFinding(finding) {
  ...
  return `${ruleLabel(finding)} (${size})`;
}

/**
 * A title-only workflow command (`::warning title=…::message`), escaped.
 *
 * @param {'error'|'warning'} command
 * @param {string} title
 * @param {string} message
 */
export function plainAnnotation(command, title, message) {
  return `::${command} title=${escapeProperty(title)}::${escapeData(message)}`;
}
...
export function summaryMarkdown({ label, findings, failOn, notices, maxRows = 100 }) {
  ...
  if (findings.length > 0) {
    ...
    for (const f of findings.slice(0, maxRows)) {
      lines.push(
        `| ${f.severity} | ${escapeMd(ruleLabel(f))} | \`${escapeMd(formatLocation(f))}\` | redacted, ${f.length ?? '?'} chars |`,
      );
    }
