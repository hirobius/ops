 ...
 *
 * Denylist entries and private-URL patterns also run on normalized copies of
 * the text (percent/JSON/HTML-decoded, accent-folded, comment prefixes dropped;
 * see lib/pii/normalize.mjs), because a name in code, a URL or JSON rarely
 * looks like the name in prose. Hits are still reported at raw offsets.
 *
 * @module pii/detect
 */

import { foldPattern, normalizeForMatching } from './normalize.mjs';
...
const EMAIL_ALLOWLIST = Object.freeze({
...
export function detectPii(text, options) {
  ...
  const findings = [];
  const copies = lazyCopies(source);
  ...
  for (const entry of denylist) {
    ...
    const folded = foldedPattern(entry);
    const matches = matchesWithCopies(entry.regex, source, folded ? copies.all() : [], folded);
    for (const match of matches) findings.push(locate(partial, match, lineStarts, firstLine));
  }

  findings.push(...denylistCopyLines(source, denylist, { path, lineStarts, firstLine }));

  if (isGenericExemptPath(path)) return findings;

  for (const { detail, regex } of PRIVATE_URL_PATTERNS) {
    for (const match of matchesWithCopies(regex, source, [copies.decoded()], regex)) {
      findings.push(
        locate(
  ...
  for (const match of matchesOf(PHONE_REGEX, source)) {
    if (isPlaceholderPhone(match.text)) continue;
    findings.push(
      locate(
...
const GENERIC_EXEMPT_PATHS = Object.freeze([
  /^fixtures\/check-pii\//,
 ...
 * @returns {boolean}
 */
function isGenericExemptPath(path) {
  const normalized = String(path ?? '').replace(/\\/g, '/');
  return GENERIC_EXEMPT_PATHS.some((re) => re.test(normalized));
}

// ── Normalized copies (lib/pii/normalize.mjs) ────────────────────────────────
//
// Denylist entries run on the raw text, a decoded copy and a loosened copy;
// private-URL patterns on the raw text and the decoded copy. A match in a copy
// is mapped back to raw offsets and dropped when it overlaps one already found,
// so a plain name is still reported once.

/** Copies are built on first use: most text never needs the loosened one. */
function lazyCopies(source) {
  let decoded;
  let loosened;
  const api = {
    decoded: () => (decoded ??= normalizeForMatching(source)),
    loosened: () => (loosened ??= normalizeForMatching(source, { loosen: true })),
    all: () => [api.decoded(), api.loosened()],
  };
  return api;
}
...
/** An entry's pattern with the same accent folding as the copies (cached). */
function foldedPattern(entry) {
  if (!FOLDED.has(entry.regex)) FOLDED.set(entry.regex, foldPattern(entry.regex));
  return FOLDED.get(entry.regex);
}

/** Raw matches of `rawRegex`, then non-overlapping matches of `copyRegex` in each copy, in raw offsets. */
function matchesWithCopies(rawRegex, source, copies, copyRegex) {
  const found = [...matchesOf(rawRegex, source)];
  for (const copy of copies) {
    if (copy.text === source && copyRegex.source === rawRegex.source) continue;
    for (const m of matchesOf(copyRegex, copy.text)) {
      const index = copy.start[m.index];
      const length = copy.end[m.index + m.length - 1] - index;
      if (length <= 0) continue;
      if (found.some((f) => index < f.index + f.length && f.index < index + length)) continue;
      found.push({ index, length, text: source.slice(index, index + length) });
    }
  }
  return found;
}
...
//
// A regex entry does not match its own source text (`jane\s+example` is not
// "jane example"), so a backup of the denylist (`.pii-denylist~`, a pasted
// list) would pass the content rules. A line that IS an entry's pattern text
// is reported instead. Literal entries match themselves and need no help.

function denylistCopyLines(source, denylist, { path, lineStarts, firstLine }) {
  const bySource = new Map();
  for (const entry of denylist) {
    const text = entry.regex.source;
    if (!new RegExp(text, 'i').test(text)) bySource.set(text.toLowerCase(), entry);
  }
  if (bySource.size === 0) return [];
  const findings = [];
  lineStarts.forEach((lineStart, i) => {
    const lineEnd = i + 1 < lineStarts.length ? lineStarts[i + 1] - 1 : source.length;
    const line = source.slice(lineStart, lineEnd);
    const trimmed = line.trim();
    const entry = trimmed && bySource.get(trimmed.toLowerCase());
    if (!entry) return;
    const where = entry.source && entry.source !== 'inline' ? ` (${entry.source})` : '';
    findings.push(
      locate(
        {
          rule: 'denylist-file',
          detail: `line is the pattern of denylist entry ${entry.index}${where}: a copy of the denylist`,
          severity: 'error',
          path,
        },
        { index: lineStart + line.indexOf(trimmed), length: trimmed.length },
        lineStarts,
        firstLine,
      ),
    );
...
/** Every non-empty match of a regex, as { index, length, text } — text stays internal, never in a finding. */
function* matchesOf(regex, text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    yield { index: m.index, length: m[0].length, text: m[0] };
  }
}
