 ...
 * @property {string} source  where the entry came from ('env' | 'file' | 'fixture' | caller label)
 ...
 * @param {(...parts: string[]) => string} [options.join] path joiner (default node:path join)
 * @returns {{ entries: DenylistEntry[], invalid: { index: number, source: string, reason: string }[], sources: string[] }}
 */
export function loadDenylist({ env, roots, readFile, join = nodeJoin }) {
  const entries = [];
  const invalid = [];
  const sources = [];
