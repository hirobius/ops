 ...
 * @property {(input: { issueUrl: string }) => Promise<LinkedPullRequest | null>} getLinkedPullRequest
 */

/**
 * @typedef {object} LinkedPullRequest
 * @property {string} url
...
export function makeGitHubPort() {
  ...
  return {
    ...
    /**
     * Resolves the most-recently-referenced pull request linked to an issue,
     * via the issue's timeline (`cross-referenced` events — the same
     * mechanism GitHub's own "Development" sidebar uses; a PR that mentions
     * `#<number>` anywhere in its body/commits shows up here, no "Closes #"
     * required). Returns null when the agent hasn't opened a PR yet — not an
     * error. Used by lib/tasks/dispatch-status.mjs to derive live board status
     * from a task's `dispatch_url`.
     * @param {{ issueUrl: string }} input
     * @returns {Promise<LinkedPullRequest | null>}
     */
    async getLinkedPullRequest({ issueUrl }) {
      const m = /github\.com\/([^/]+)\/([^/]+)\/issues\/(\d+)/.exec(issueUrl || '');
      if (!m) {
        throw new Error(
          `getLinkedPullRequest: could not parse owner/repo/issue number from URL: ${issueUrl}`,
        );
      }
      const [, owner, repoName, number] = m;
      const res = await fetch(
        `https://api.github.com/repos/${owner}/${repoName}/issues/${number}/timeline`,
        { headers: GH_HEADERS(token), signal: AbortSignal.timeout(9000) },
      );
      if (res.status === 401 || res.status === 403) throw new Error(authHint(res.status));
      if (!res.ok) {
      ...
      return pickLatestLinkedPr(await res.json());
    },
  };
}

/**
 * Pure: pick the most recent cross-referenced PR out of an issue timeline
 * response. Exported for unit testing without a fetch mock.
 * @param {Array<{ event: string, source?: { issue?: any } }>} events
 * @returns {LinkedPullRequest | null}
 */
export function pickLatestLinkedPr(events) {
  const linked = (Array.isArray(events) ? events : [])
    .filter((e) => e?.event === 'cross-referenced' && e?.source?.issue?.pull_request)
    .map((e) => e.source.issue);
  if (!linked.length) return null;
  const latest = linked[linked.length - 1]; // timeline is chronological
  return {
    url: latest.html_url,
    number: latest.number,
    state: latest.state,
    merged: !!latest.pull_request.merged_at,
    draft: !!latest.draft,
  };
}
