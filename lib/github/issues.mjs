 ...
 * @property {(input: { repos: string[] }) => Promise<Array<{ repo: string, issues: Array<{ repo: string, number: number, title: string, url: string, labels: string[], body: string, parkedComment: string|null }>, error?: string }>>} listParkedIssues
 * @property {(input: { repos: string[] }) => Promise<Array<{ repo: string, prs: Array<{ number: number, title: string, url: string, updated_at: string, gate: string }>, error?: string }>>} listOpenRalphPrs
 */
...
/** Open issues (not PRs) carrying `label` on one repo, normalized like `listRalphReadyIssues`. */
async function fetchLabeledIssues(token, fullRepo, label) {
...
/**
 * The latest 🅿️ park comment body on an issue, or null if it's never been
 * parked. `park_issue` (ralph/lib.sh:281-295) always posts in this format,
 * for both the `ralph-parked` and `needs-adrian` label paths.
 */
async function fetchLatestParkedComment(token, owner, repo, number) {
  const res = await fetch(
  ...
  const parked = comments.filter((c) => typeof c.body === 'string' && c.body.startsWith('🅿️'));
  return parked.length ? parked[parked.length - 1].body : null;
}

/**
 * Latest `ralph-gate` combined-status state for a commit — the same lookup
 * `classify_wedged` makes (ralph/lib.sh:117-119). Returns 'none' on any
 * fetch failure so one bad status lookup doesn't sink the whole PR list.
 */
async function fetchGateStatus(token, fullRepo, sha) {
  try {
    ...
    const body = /** @type {{ statuses?: any[] }} */ (await res.json());
    const gateStatuses = (body.statuses ?? [])
      .filter((s) => s.context === 'ralph-gate')
      .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
    return gateStatuses.length ? gateStatuses[gateStatuses.length - 1].state : 'none';
  } catch {
    return 'none';
  }
}
...
export function makeGitHubPort() {
  ...
  return {
    ...
    /**
     * Open issues per fleet repo labeled `ralph-parked` OR `needs-adrian`
     * (ops#141's parked-inbox lane) — GitHub ANDs comma-separated `labels`
     * filters, so this is two label fetches merged by issue number, not one
     * query. Each issue carries its latest 🅿️ park comment (if any) so the
     * panel can show the reason without a second round-trip per row.
     */
    async listParkedIssues({ repos }) {
      return Promise.all(
        repos.map(async (fullRepo) => {
          try {
            const [parked, needsAdrian] = await Promise.all([
              fetchLabeledIssues(token, fullRepo, 'ralph-parked'),
              fetchLabeledIssues(token, fullRepo, 'needs-adrian'),
            ]);
            const byNumber = new Map();
            for (const i of [...parked, ...needsAdrian]) byNumber.set(i.number, i);
            const [owner, repoName] = fullRepo.split('/');
            const issues = await Promise.all(
              [...byNumber.values()].map(async (i) => ({
                ...i,
                parkedComment: await fetchLatestParkedComment(token, owner, repoName, i.number),
              })),
            );
    ...
    /**
     * Open Ralph PRs (branch prefix `ralph/`, excluding the atomic-claim
     * `ralph/claim-*` branches — same filter as `open_ralph_prs`,
     * ralph/lib.sh:102-106) per fleet repo, each carrying its latest
     * `ralph-gate` status so `classifyWedged` (lib/tasks/ralph-wedge.mjs) can
     * badge wedged ones on the panel (ops#141).
     */
    async listOpenRalphPrs({ repos }) {
      return Promise.all(
        repos.map(async (fullRepo) => {
          try {
            const res = await fetch(
            ...
            const withGate = await Promise.all(
              ralphPrs.map(async (pr) => ({
                ...
                gate: await fetchGateStatus(token, fullRepo, pr.head.sha),
              })),
            );
            return { repo: fullRepo, prs: withGate };
          } catch (err) {
            return {
              repo: fullRepo,
              prs: [],
