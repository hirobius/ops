 ...
 * @property {(input: { owner: string, repo: string, issueNumber: number|string }) => Promise<Array<{ number: number, url: string, state: 'open'|'closed', merged: boolean, created_at: string }>>} getLinkedPullRequest
...
export function makeGitHubPort() {
  ...
  return {
    ...
    /**
     * Every pull request cross-referenced onto an issue's timeline (ops#107) —
     * the same signal GitHub's own "Development" sidebar surfaces, so this
     * finds a linked PR regardless of branch-naming convention (unlike
     * `findRalphPr`, which only matches the Ralph loop's `ralph/issue-<n>-`
     * branches — a `@claude`-dispatched task's PR branch doesn't follow that
     * shape). Returns every cross-referenced PR (a task can accumulate more
     * than one over retries); callers pick the one they want (see
     * `lib/tasks/dispatch-status.mjs::pickLatestLinkedPr`). Non-PR
     * cross-references (a linked issue, not a PR) are filtered out.
     * @param {{ owner: string, repo: string, issueNumber: number|string }} input
     */
    async getLinkedPullRequest({ owner, repo, issueNumber }) {
      const res = await fetch(
      ...
      const events = /** @type {any[]} */ (await res.json());
      return events
