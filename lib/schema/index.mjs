...
import { CONTENT_PACKS } from './content-packs.mjs';
...
/**
 * Merge a content pack's defaults under a draft config (site-engine#118).
 * Override-only: any field the draft sets explicitly wins, the pack only fills
 * gaps. Returns a plain config input (the `contentPack` key never reaches Zod —
 * it isn't part of the canonical `ClientConfigSchema` shape, so the schema
 * drift guard is unaffected by picking a pack).
 * @param {any} config
 */
function applyContentPack(config) {
  if (!config || !('contentPack' in config)) {
    return config;
  }
  ...
  if (!pack) {
    throw new Error(
      `Unknown contentPack "${contentPack}" — expected one of ${PALETTE_PRESET_IDS.join(', ')}`,
    );
  }
  return {
    ...rest,
...
export * from './content-packs.mjs';
export * from './lead-to-config.mjs';
