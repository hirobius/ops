 ...
 *   identifiers: { id?: string, email?: string, phone?: string, placeId?: string },
  ...
  if (!Object.values(want).some(Boolean)) {
    throw new Error(
      'privacy request: give at least one usable identifier — a lead id, a Google place id, ' +
        'an email address, or a phone number with its area code.',
    );
  }
...
function normalizeIdentifiers({ id, email, phone, placeId } = {}) {
  return {
    id: normText(id),
...
function matches(row, want) {
  return (
    ...
    (Boolean(want.email) && normText(row.email).toLowerCase() === want.email) ||
