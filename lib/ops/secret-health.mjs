...
/** Registered names no fleet workflow references any more — the other half of "agree". */
export function findOrphanedSecrets(registryNames, grepNames) {
  const live = new Set(grepNames);
  return [...new Set(registryNames)].filter((n) => !live.has(n)).sort();
}

/** Both directions of the registry-vs-workflow check, as gate violations. */
export function buildSyncViolations(registryNames, grepNames) {
  const violation = (name, rule, message) => ({
    file: '*',
    line: null,
    rule,
    severity: 'error',
  ...
  return [
    ...findUnregisteredSecrets(registryNames, grepNames).map((n) =>
      violation(
        n,
        'SECRET_NOT_REGISTERED',
        'is referenced in a fleet workflow but not in docs/secrets/registry.json.',
      ),
    ),
    ...findOrphanedSecrets(registryNames, grepNames).map((n) =>
      violation(
        n,
        'SECRET_NOT_REFERENCED',
        'is in docs/secrets/registry.json but no fleet workflow references it — remove the entry, or the secret itself if it is dead.',
      ),
    ),
  ];
}
