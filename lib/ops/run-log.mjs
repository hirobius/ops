 ...
 *   tokens       optional — total token count for the run, if known (a
 *                finite, non-negative number — not every caller has this;
 *                fleet-dispatch.mjs's dispatch/queue recaps never do, since
 *                it only opens/flips GitHub issues, it doesn't run a model)
 *
...
const OPTIONAL_STRING_FIELDS = ['task', 'model', 'tier', 'session_url'];
 ...
 *   task?: string, model?: string, tier?: string, session_url?: string,
 *   tokens?: number }} entry
