  ...
  if (openRalphPrs.length > 0) {
    ...
    if (pr.gateConclusion === 'success') {
      ...
      return {
        ...
        ...extra,
        reason:
          `PR #${pr.number} has ralph-gate green but did not auto-merge, so the auto-merge ` +
          `did not arm. Confirm its issue carries ralph-auto, then merge using the FULL head ` +
          `SHA — GitHub's merge API rejects a short SHA.`,
      };
    }
