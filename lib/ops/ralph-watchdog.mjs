...
/**
 * Why the watchdog chose to do nothing. Distinguishing these matters: most are
 * healthy waits; "queue-empty" is the signal that the backlog is exhausted and
 * a human should decide what is next; and "awaiting-approval" holds the
 * single-flight queue on a human, so it carries `needsHuman` (see below) rather
...
export const IDLE_REASON = {
  ...
  /** GitHub has not finished computing `mergeable` — the next tick re-reads it. */
  MERGEABILITY_PENDING: 'mergeability-pending',
};

/**
 * Why a PR's diff could not be checked against the supervised paths. The two
 * differ in what happens next, so the reason text must not conflate them: an
 * API error is transient and the next tick retries; the file cap is permanent,
 * because GitHub's PR-files API never lists more than 3000 files.
 * @readonly
 */
export const DIFF_UNREADABLE = {
  API_ERROR: 'api-error',
  FILE_CAP: 'file-cap',
};
 ...
 *           the `ralph-gate` COMMIT STATUS only — the check branch protection
 *           requires. Not the engine's Actions check run, which exits 0 even
 *           when it sets the status to failure. Other CI jobs are informational
 *           and MUST NOT be folded in here — a PR is mergeable with them red.
 * @property {boolean|null} [mergeable]   from the SINGLE-PR endpoint (the list
 *           endpoint never carries it). `null`/absent means GitHub has not
 *           computed it yet — never merged on unknown; `false` is a conflict.
 * @property {string} [mergeableState]     same source; 'dirty' means conflict
 ...
 * @property {string[]|null} [supervisedFiles] paths in the diff under a
 *           supervised (revenue) path, per ops#238 — a rename counts on BOTH its
 *           new and its previous path. `null`/absent means the diff could not
 *           be read in full — never merged on unknown.
 * @property {string|null} [diffUnreadable] one of DIFF_UNREADABLE when
 *           `supervisedFiles` is null; says whether a retry can help
 ...
 * @property {RalphPr[]} openRalphPrs   open PRs from a `ralph/*` branch IN THIS
 *           repo — never a fork's, whatever its branch is named
 ...
 * @property {boolean} [needsHuman] an IDLE that only a human can clear (a
 *           supervised diff, or one too large to read). The caller asks, once per
 *           head SHA — an unannounced wait is the silent wedge this module exists
 *           to catch. Never set for a wait the next tick can clear by itself.
 */
...
export function decideWatchdogAction(facts) {
  ...
  if (openRalphPrs.length > 0) {
    ...
    // GitHub only sets `mergeable: false` for a conflict.
    if (pr.mergeableState === 'dirty' || pr.mergeable === false) {
      return {
    ...
    if (pr.gateConclusion === 'success') {
      ...
      if (pr.prApproved !== true) {
        ...
        if (!Array.isArray(supervised)) {
          const reason =
            pr.diffUnreadable === DIFF_UNREADABLE.FILE_CAP
              ? `PR #${pr.number} has ralph-gate green, but its diff reaches GitHub's ` +
                `${PR_FILES_API_CAP}-file listing cap, so the watchdog cannot see every file to ` +
                `check the supervised revenue paths (ops#238) and will not merge on unknown. ` +
                `Retrying cannot help: it blocks the queue until a human reviews it and adds ` +
                `ralph-approved, or splits it into smaller PRs.`
              : `PR #${pr.number} has ralph-gate green, but the watchdog could not read its diff` +
                `${pr.diffUnreadable === DIFF_UNREADABLE.API_ERROR ? ' (GitHub API error)' : ''} ` +
                `to check the supervised revenue paths (ops#238), so it will ` +
                `not merge on unknown. The next tick retries; a human can also review it and ` +
                `add ralph-approved.`;
          return {
            action: ACTION.IDLE,
            pr: withIssue,
            ...
            // A transient API error clears itself on retry; the file cap never does.
            ...(pr.diffUnreadable === DIFF_UNREADABLE.FILE_CAP ? { needsHuman: true } : {}),
            reason,
          };
        }
        if (supervised.length > 0) {
          ...
          return {
            ...
            needsHuman: true,
            reason:
              `PR #${pr.number} has ralph-gate green but touches supervised files ` +
              `(${shown}${more}), which need a human (ops#238). It blocks the queue until ` +
              `someone reviews it and adds ralph-approved.`,
          };
        }
      }
      // Merging on an uncomputed mergeability 405s. Waiting one tick does not.
      if (pr.mergeable !== true) {
        return {
          action: ACTION.IDLE,
          pr: withIssue,
          ...extra,
          idleReason: IDLE_REASON.MERGEABILITY_PENDING,
          reason:
            `PR #${pr.number} has ralph-gate green and clears the supervised-path boundary, but ` +
            `GitHub has not finished computing whether it can merge. Reading the PR starts that ` +
            `computation; the next tick re-reads it and merges or reports the conflict.`,
        };
      }
      return {
        action: ACTION.MERGE,
 ...
 * An IDLE that `needsHuman` counts: asking the human is itself a write.
 * @param {WatchdogDecision} decision
 * @returns {boolean}
 */
export function isMutating(decision) {
  return decision?.action !== ACTION.IDLE || decision?.needsHuman === true;
}
