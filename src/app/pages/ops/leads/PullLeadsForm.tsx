  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (response.ok && typeof data.inserted === 'number') {
        ...
        setStatus({ kind: 'error', message: data.error ?? `Request failed (HTTP ${response.status})` });
      }
    } catch (error) {
      ...
      <Stack direction="column" gap="gap">
        ...
        {status.kind === 'done' && (
          <Stack direction="row" gap="gap" align="center" wrap="wrap">
            ...
            <span style={statusTextStyle}>{status.inserted} lead{status.inserted === 1 ? '' : 's'} upserted.</span>
          </Stack>
        )}
        {status.kind === 'error' && (
          <Stack direction="row" gap="gap" align="center" wrap="wrap">
