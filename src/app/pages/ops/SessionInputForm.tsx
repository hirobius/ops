  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (data.code === 0 && data.result?.verdict === 'task') {
        ...
        setStatus({
          kind: 'error',
          message: data.stderr ?? `Assigner exited with code ${data.code}`,
        });
      }
    } catch (error) {
      ...
      <Stack direction="column" gap="gap">
        ...
        <Stack direction="row" gap="gap" align="center" wrap="wrap" justify="space-between">
          ...
          <button
            ...
            style={
              status.kind === 'sending' || !text.trim() ? sendButtonDisabledStyle : sendButtonStyle
            }
          >
...
function SubmitStatusBanner({
  ...
  status: Exclude<SubmitStatus, { kind: 'idle' } | { kind: 'sending' }>;
}) {
  if (status.kind === 'task') {
    ...
    return (
      <Stack direction="row" gap="gap" align="center" wrap="wrap">
        ...
        <span style={statusTextStyle}>
          <code style={inlineCodeStyle}>{r.taskId}</code>
          {' → '}
          <code style={inlineCodeStyle}>{r.model}</code>
          {r.tier ? (
            <>
              {' · '}
              <code style={inlineCodeStyle}>{r.tier}</code>
            </>
          ) : null}
        </span>
      </Stack>
