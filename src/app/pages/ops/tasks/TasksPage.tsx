    ...
    <div style={s.page}>
      ...
      <div style={s.controls}>
        ...
        <span style={s.statusLine}>
          {isOffline ? 'offline' : isInitialLoading ? 'loading…' : `${summary} · updated ${formatLastUpdated(lastUpdatedAt)}`}
        </span>
        ...
        byLane.map(([lane, laneTasks]) => (
          <section key={lane} style={s.lane} aria-labelledby={`lane-${lane}`}>
            <header style={s.laneHeader}>
              <span id={`lane-${lane}`} style={s.laneLabel}>{lane}</span>
                ...
                return (
                  <li key={t.key} style={s.row}>
                    ...
                    <div style={s.rowAside}>
                      ...
                      {dispatched && (
                        <a href={t.dispatch_url ?? '#'} target="_blank" rel="noreferrer" style={s.linkAction}>issue ↗</a>
                      )}
                      <Badge tone={STATUS_TONE[t.status] ?? 'neutral'}>{t.status}</Badge>
                      {t.status === 'done' ? (
                        <button type="button" disabled={busy} onClick={() => act(t.key, 'reopen')} style={busy ? s.btnDisabled : s.btn}>
                          {busy ? '…' : 'Reopen'}
                        </button>
                      ) : (
                        <>
                          <button type="button" disabled={busy} onClick={() => act(t.key, 'done')} style={busy ? s.btnDisabled : s.btn}>
                            {busy ? '…' : 'Done'}
                          </button>
                          {!dispatched && (
                            <button type="button" disabled={busy} onClick={() => act(t.key, 'dispatch')} style={busy ? s.btnDisabled : s.btnPrimary}>
                              Dispatch
                            </button>
                          )}
                        </>
                      )}
                      <button type="button" disabled={busy} onClick={() => act(t.key, 'trash')} style={busy ? s.btnDisabled : s.btnGhost} aria-label="Trash task">
                        ✕
                      </button>
                    </div>
                  </li>
...
const s = {
  ...
  rowMain: { display: 'flex', flexDirection: 'column' as const, gap: hds.space.px4, minWidth: 0, flex: '1 1 18rem' },
  title: { ...hds.typeStyles.ui, color: 'var(--semantic-color-content-primary)' },
  meta: { fontFamily: hds.monoFamily, fontSize: hds.fontSize.xs, color: 'var(--semantic-color-content-secondary)' },
  rowAside: { display: 'flex', alignItems: 'center', gap: hds.space.px8, flexWrap: 'wrap' as const },
  claim: { fontFamily: hds.monoFamily, fontSize: hds.fontSize.xs, color: 'var(--semantic-color-content-secondary)' },
  linkAction: { ...hds.typeStyles.ui, fontSize: hds.fontSize.xs, color: 'var(--semantic-color-content-accent)', textDecoration: 'none' },
  btn: {
    ...hds.typeStyles.ui,
    fontSize: hds.fontSize.xs,
    padding: '4px 10px',
